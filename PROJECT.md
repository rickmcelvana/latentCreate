...
- **`server_died` is observed but unrendered** (MCP-SURFACE 27.3). The store produces it; no app job
  has been live across a crash, so T-310b's failure line stays provisional and T-314 still owes the
  kill-mid-job check.
...
**Then: T-312, batch by seeds.** The phase file describes it as N jobs from one spec differing only
