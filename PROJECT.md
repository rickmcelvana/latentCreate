...
**T-403 (album lists, the second milestone line) has landed (a/b/c)** -- `library::albums`
(create/rename/add/remove/reorder over `Project.albums`), the six `albums_*`/`album_*` commands,
the `state/albums` store with pure `albumRows`/`moveTrackId` (18 tests), and the Library album
panel. Implemented 2026-08-31; the producer click-through is the remaining checkpoint before the
milestone line discharges. T-404 … T-406 planned, not briefed.
