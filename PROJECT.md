...
- **Next up (2026-09-06) -- read this first if you are the next session.** **All feature work in
  ...
  - **T-508 -- installer builds.** The **phase milestone** and the only thing standing between this
    repo and someone who is not the developer using it. Windows first (NSIS/MSIX), then macOS `.dmg`
    and Linux AppImage via CI. The milestone check is written in [tasks/phase-5.md](tasks/phase-5.md):
    a person on a machine that never had the dev toolchain installs the build, installs a curated
    image model from Setup, and generates cover art with a provenance sidecar. **Its in-app half is
    already discharged and owner-verified** -- only the build half is missing.
  ...
  **Order:** T-508 first (it is the milestone and the riskiest -- CI, signing, three platforms), then
  T-509 and T-510 alongside it, and T-515 in its own session whenever the owner wants it; it blocks
  the version, not the phase milestone.
