...
- **2026-08-28 -- `nodes(action="get")` succeeds with ComfyUI down, and the wrapper was hiding it.** comfy-cli serves the class from its own `object_info` cache and flags it with `stale: true` plus an `object_info_stale` warning; `mcp-bridge` decoded neither, so every caller treated a cache as the installed truth. Now decoded and **tri-state like `local_check`** -- `Some(true)` cached, `Some(false)` live, `None` the response did not say, and `None` is not fresh. Only the cached shape has been observed live; whether a fresh read sends `stale: false` or omits the key is unverified and T-314 settles it for free. **Why it matters:** the same call enumerates LoRAs, where a cached list offers files the user deleted and hides the one they just trained -- and picking a deleted one does not fail, it writes a track with no LoRA on it (MCP-SURFACE 17.6). MCP-SURFACE 19.1.
...
cached list offers files the user deleted and hides the one they trained an hour ago -- and
picking a deleted one does not error, it completes and writes a track with no LoRA on it
(17.6). That is 17.1's failure reached through a cache rather than a bad splice, so the warning
had to exist **before** T-309 builds the picker on the same path.
