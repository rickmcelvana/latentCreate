...
- **Next up (2026-09-05):** **T-507b** -- the last piece of T-507. The frontend half is done: **T-507a** (the empty/degraded-states sweep) and **T-507a-2** both landed and passed click-through 2026-09-05 -- the Lyrics view now flags a missing *or* configured-but-unreachable model (reusing Setup's probe, no new mount-probe) and disables Generate/Optimize for the states that can't work, and Audio shows a cold-start line instead of a bare picker. Setup/Library/CoverArt already handled their states and were left alone. **T-507b** is the named carry-over from T-505d-d's click-through: an imported/adopted profile declares no `comfy.models`, so the Setup Models row reads "Cannot check / Undeclared". The fix populates `comfy.models` in Rust `emit` from the graph's loader COMBO slots (Klein: `75/70.unet_name`, `75/71.clip_name`, `75/72.vae_name`), resolving each file's folder via `search_models` -- so an adopted row reads **Ready** without the app ever claiming it can fetch someone else's weights (`source_url`/`size_bytes` stay `None`). Then **T-508** installers, **T-509** THIRD-PARTY-LICENSES, **T-510** public-repo readiness. No feature work remains in the phase.
