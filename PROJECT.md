...
- **Phase:** **0-3 complete**, tagged `phase0-done` (2026-08-23), `phase1-done` (2026-08-25), `phase2-done` (2026-08-26); Phase 3 closed 2026-08-30 (T-301 ... T-317, all five milestone lines discharged -- the pipeline generates audio live on both profiles, ingests a finished track with a provenance sidecar checked field-for-field against the executed graph, batches by seed, imports a user's own workflow, and fails cleanly on a mid-job crash). **Phase 4 (Library & Player) is COMPLETE -- closed 2026-09-02** -- [tasks/phase-4.md](tasks/phase-4.md). Every task landed and passed its own producer click-through: T-401 (projects first-class), T-402 (playback + AnalyserNode visualizer), T-403 (album lists), T-404 (Send-to link-out), T-405 (track actions: delete-to-trash / rename / export / reveal), T-407 (shared scrollbar CSS), T-408 (delete for every kind of created content), T-409 (the song title carried to the track and the exported file), T-406 (provenance inspector + "re-use these settings"). The milestone check (generate -> play with visualizer -> album list -> send-to) was met by 2026-09-01; the last three tasks were owner-decision additions. **Not git-tagged** (Phase 3 set the recent precedent of a docs-only close). **Phase 5 (model catalog, cover art, polish, packaging -- T-501...) is OPEN, 2026-09-02** -- [tasks/phase-5.md](tasks/phase-5.md). Its **phase-start check is done** (the image/catalog comfy surface re-verified live: [docs/MCP-SURFACE.md §32](docs/MCP-SURFACE.md)) and the task map is laid out. **Landed so far: T-501/T-502/T-503 polish; the catalog arc (T-504/T-505 built a gallery browser + adopt, then the 2026-09-05 pivot made the catalog a curated installable list and T-512 REMOVED the gallery browse/adopt -- see decisions log); T-506 cover art; T-507 first-run/empty-state polish; T-511 five curated image profiles; T-513 two more audio profiles (ACE Base/SFT).** The T-505d "Bring in"/adopt UI described in older log entries no longer exists. **Adopting a gallery model into a working profile now works end to end** -- verified 2026-09-03 by adopting Flux.2 Klein 9B from the catalog and reading the emitted profile field by field. **T-506 (cover art) is COMPLETE 2026-09-03, all five lanes landed and both click-throughs passed**, and was split into five lanes, scoped 2026-09-03 by running the adopted Klein profile end to end **outside the app** -- the same sequence `build_and_submit` performs, then `GET /history` to read what executed (docs/MCP-SURFACE.md 35). It found the Rust generation half already works unchanged for an image profile (the lossless swap no-ops, the LoRA splice is skipped, and the prompt/negative/seed/steps writes all reach the engine), that what is missing is everything **after** the output (ingest filters on `AUDIO_EXTS` and drops a PNG **silently**, so an image job today reports Done and saves nothing), and that every size-shaped slot on Klein is inert -- the effective address is a `PrimitiveInt`, proved by writing 1536 to the obvious ones and 768 to the primitives and measuring the PNG at 768x768. Lanes: **a** the artwork record + storage and **b** `generate_image` + art ingest (both **landed 2026-09-03**, so a cover art job now files an `art/<id>.png` with a provenance sidecar), **c** the art stores (**c-a, c-b and c-c all landed 2026-09-03** -- the config field and the two read commands, the panel factory and the art submit store, and the artwork listing store), **d** the CoverArt view (**landed 2026-09-03; click-through passed, all 11 steps**), **e** attach-to-track/album + `delete_art`, **split into e-a (the backend, landed 2026-09-03), e-b (the cover stores, landed 2026-09-03) and e-c (the views, landed 2026-09-03, click-through passed) -- all complete**. **Cover art works end to end as of 2026-09-03**: pick an image model, generate, and a finished PNG files itself into `art/` with a provenance sidecar and appears in the gallery with no reload. The phase's centrepiece is a **model catalog** on Setup -- an owner-flagged plan gap: it was in the original plan, survived only as a mis-stated half-line in ARCHITECTURE §10, and was never built. Verified 2026-09-02: comfy-mcp has **no model-hub search**, so the catalog surfaces the built-in **template gallery** (`search_templates`) as browse-and-install for **both audio and image** models, adopting each through the existing T-313 import-to-profile path; **local-only for v1** (the paid `list_partner_models` cloud tier is backlog). Three Phase-4 polish items the producer flagged are pulled in: the lyrics doc-picker floats without a card (T-501), the Library player sits off-screen at the page foot (T-502), and the visualizer blurs in full screen because a fixed 640x120 canvas is upscaled to `width:100%` (T-503). Counts (2026-09-06, live gate): **create-core 206, library 143, mcp-bridge 98, llm-bridge 35, src-tauri (`app`) 130, frontend 552**; `npm run gate` green, tree clean. **Nine curated profiles ship** in `profiles/` (audio: ace-step-1.5-turbo/-xl-base/-xl-sft, minimax-music-3; image: flux-1-schnell-fp8, chroma-1-hd, sdxl-base-1.0, sd3.5-large-fp8, qwen-image). **T-514 (the 2026-09-06 Setup/studio design pass) landed** -- Setup now owns choosing, installing and explaining models (one shared scrolling `.picker-list` for music models, image models and projects, with Install in the row), and Audio/Cover Art/Lyrics/Library carry a name-only `QuickSwap` dropdown over what is installed; Library's Play button leads the row and the loaded track is tagged. **T-516** followed: an exported track now carries title, artist, album, track number, year, a model/licence comment and its cover, written onto the copy only. **Every outstanding producer click-through PASSED 2026-09-06 -- T-512, T-514 and T-516 -- so nothing in this repo is now waiting on the owner.** Per-task delta history is in the session log below, not here.
- **Next up (2026-09-07) -- read this first if you are the next session.** **All feature work in
  ...
  - **T-508 -- installer builds. ✅ LANDED 2026-09-06** ([brief](tasks/t-508-brief.md)). A
    ...
    **draft** GitHub release. **Unsigned** (owner decision): no code signing, no MSIX -- SmartScreen
    and Gatekeeper will warn. `tauri.conf.json` gained `bundle.macOS.signingIdentity: "-"` (ad-hoc
    signing, so an Apple Silicon build is not reported "damaged"). The milestone's in-app half was
    already discharged; the build half is now in place and awaits the owner's one `v*` tag push to
    produce the four installers.
  - **T-509 -- THIRD-PARTY-LICENSES.** Must carry the Rust and npm dependency licences, **all nine
    curated model licences**, and now **`lofty` (MIT OR Apache-2.0)**, added by T-516.
  - **T-510 -- public-repo readiness.** CONTRIBUTING, issue/PR templates, a README pass for a reader
    who has never seen the project.
  - **T-515 -- reaching a ComfyUI that is not on this machine.** A **v1 blocker** the owner has raised
    repeatedly, named 2026-09-06. Its own session, because the live surface check reshapes screens
    rather than adding a field: `ComfyMode`/`ComfyConfig.url`/`comfy_cloud_api_key` all exist in the
    schema and **nothing reads them**, comfy-mcp's `comfy_target` is a different product from Comfy
...
- **2026-09-05 -- the audio catalog gets the ACE variants people actually use; Stable Audio is
  deferred, not skipped.** T-511 curated five image models but left audio at the two that shipped --
  ACE **Turbo** (chosen for being small/fast to test) and MiniMax -- which the owner flagged as too
  thin. **T-513** adds **ACE-Step 1.5 XL Base and SFT** (Apache-2.0, the non-distilled variants most
  users run), each a clone of the Turbo profile with a swapped unet (`acestep_v1.5_xl_base_bf16` /
  `_sft_bf16`, both 9,974,719,930 bytes, ungated) and a 50-step default; they share Turbo's encoders,
  VAE, lyrics contract, prompt guide and LoRA ecosystem. **Stable Audio was tried and deferred** after
  two verified dead-ends: **(1)** the **SA 3.0** gallery template is an LLM-prompt-expansion subgraph
  comfy-cli flags with a `suspect_slots` pairing warning -- `set_workflow_slot` cannot drive it
  reliably; **(2)** **SA Open 1.0**'s checkpoint is clean and ungated (`stable-audio-open-1.0.safetensors`,
  4,853,889,016 bytes, Stability Community licence) but its `t5-base.safetensors` encoder has **no
  ungated ComfyUI-ready source** (only `google-t5/t5-base/model.safetensors` -- wrong format/filename,
  unverified against ComfyUI's `stable_audio` CLIP loader). Rather than ship a guess, both are backlog:
  revisit when a slot-friendly SA template or a clean `t5-base` lands. Also considered and excluded:
  **YuE** (docs/RESEARCH.md's closest-to-Suno model) is not a gallery template (needs a hand-authored
  workflow) and is 24 GB+; **MusicGen** is CC-BY-NC (non-commercial, docs/MODELS.md).
...
- **2026-09-05 -- a degraded-state cue reuses a probe already taken; it does not take its own.**
  ...
  section 10 rule the owner set in Setup: a view that probes on mount spawns work behind the user's
  back and, on macOS, can raise a keychain prompt. So the Lyrics view reads the **llm store's
  existing `status`** (Setup's LlmStep populates it, and the app opens on Setup, so it is there from
  launch) and decides `none`/`unreachable`/`not-offered`/`ready`/`unknown` from that. The `unknown`
  branch is the honesty: when no probe has been read, the view does **not** assert a failure and
  preserves prior behaviour, rather than pretending certainty it does not have. Same principle as
  T-302b -- do not manufacture a verified fact you have not verified -- applied to a UI cue.
- **2026-09-03 -- an image model becomes a profile through the *existing* import mechanism, not a
  bespoke one.** Scoping the adopt UI found `emit::build_profile` was audio-only: it refused any
  graph without an *audio* save node and hardcoded `kind: Music`, so adopting Flux.2 Klein 9B (a
  `SaveImage` graph) would have worked up to Save and then failed. The fork was **generalize emit**
  or **hand-author a curated image profile**. Owner chose to generalize (T-505d-b): emission now
  detects the output kind from the graph's save node. The reason is ARCHITECTURE 5b's -- there is
  **one** bring-your-own mechanism, and a second, parallel path for images would have to be kept in
  step with it forever.
- **2026-09-03 -- conditioning polarity outranks the name table for the two prompt roles.** An image
  graph's positive and negative encoders are both `CLIPTextEncode` exposing an input named `text`
  (MCP-SURFACE 34.2). Name-and-type matching rates **both** `Strong`, the import screen pre-ticks
  every `Strong` candidate, and the emitted profile would write the user's prompt into the negative
  conditioning too -- silently, since it still generates. So `roles.rs` reads the graph one hop
  forward (`audit::output_targets`) and lets the link into `positive`/`negative` decide. **One hop,
  exact names, no opinion when ambiguous**: a deeper walk would follow ACE-Step's encoder through
  `ConditioningZeroOut` and call it negative too. Provably inert for audio -- both shipped models
  zero their negative side, and `ConditioningZeroOut` exposes no `STRING` slot -- which is a
  regression test, not an assumption. This is the §18.1 lesson again: **the slot list is not enough
  to decide what a slot is.**
- **2026-09-03 -- bare rows adopt, curated rows install.** A catalog row the app ships a profile for
  already has one-click install and a hand-verified file list; adopting it would emit a *second*,
  worse profile for a model the app already describes. So "Bring in" appears only on a **ready bare**
  row. The backstop is real rather than cosmetic: `import_into`'s validation refuses an un-installed
  template by name, so the readiness gate on the button is an affordance, not the safety.
- **2026-09-02 -- the model catalog was a dropped plan item; it rides the template gallery, and
  v1 is local-only.** Opening Phase 5, the owner flagged that a searchable **model catalog** -- the
  point of the Setup page beyond LLM selection, letting a user bring in audio *and* image models
  without hand-importing a workflow -- was in the original plan but had survived only as a half-line
  in ARCHITECTURE §10 ("full `search_models` search behind an advanced expander") and **was never
  built**. That half-line was also **wrong**: the live re-verification (MCP-SURFACE §32) found
  `search_models` is a **local-readiness** call (filenames on disk), not a discovery search, and
  comfy-mcp exposes **no model-hub search at all**. The only browsable-before-install surface is the
  built-in **workflow-template gallery** (`search_templates`, ~558 rows), where each row is a
  *workflow + model* bundle -- and it is the same gallery Phases 1-3 already used for audio, so one
  surface serves both audio and image. Three decisions followed. **(1)** The catalog *is* that
  gallery, filtered `api:false` and split by `output_type`; readiness is `fetch_template`/
  `local_check`, install is `download_model`. **(2)** Adoption reuses the **T-313 import-to-profile**
  path -- a gallery template is a workflow the user did not have to find -- so no second import
  mechanism is built and provenance stays honest (copy-not-reference). **(3) Local-only for v1:** the
  paid hosted/cloud tier (`list_partner_models`/`partner_generate` -- Flux Pro, Ideogram, Seedance;
  spends account credits, downloads nothing) is deferred to the backlog, keeping the "ship no models,
  runs on the user's own ComfyUI" shape clean and introducing no raw HTTP (**OQ-3 stays NO**).
  Design in ARCHITECTURE §10a; task map in tasks/phase-5.md. Also pulled into Phase 5: three Phase-4
  polish items the producer flagged (doc-picker card, on-screen player, sharp visualizer).
...
- **2026-09-06 -- Setup is where you choose; the studios are where you swap.** An owner design pass
  after a stretch of real use. Two complaints, one cause: the Setup Models step looked nothing like
  the studios' model pickers (different row layout, no scrolling window), and each studio carried a
  **full** model list -- licence, VRAM claim, readiness pill, origin -- on a screen whose job is to
  generate. The split is now by *purpose*, not by screen: **Setup chooses, installs and explains**
  (one scrolling `.picker-list` for music models, image models and projects, radio-selected, with
  Install and the licence on every row); **the studios swap** (a name-only dropdown over what is
  **installed**, on Audio, Cover Art, Lyrics and Library). One consequence recorded because it will
  be asked again: the studio dropdown writes the **same config field** the Setup radio does (owner
  decision) -- one value, so the two screens can never disagree about which model is selected, and a
  swap survives a restart. The project list, its create form and delete moved from Library to Setup,
  and `ImportWorkflow` moved from Audio to Setup, on the same rule. **T-514**; the ComfyUI-over-the-
  network gap the same pass surfaced is **T-515**, below.
- **2026-09-06 -- reaching a ComfyUI that is not on this machine is a v1 blocker (T-515).** The
  owner has raised this repeatedly and it has never been scheduled. The finding that makes it small
  enough to schedule: **the config schema already has it and nothing uses it** -- `ComfyMode::{Local,
  Cloud}`, `ComfyConfig.url`, and a whitelisted `comfy_cloud_api_key` keychain secret exist in
  `crates/library/src/config.rs`, and the only code that reads `ComfyMode` is that file's own tests.
  `src-tauri/src/comfy.rs` builds a `LocalComfy` unconditionally. The task is wiring plus a live
  surface check, **not** a redesign -- but the check comes first, because two of its answers reshape
  screens: comfy-mcp's `comfy_target` (a self-hosted remote) is **not** the same product as Comfy
  Cloud and `ComfyMode::Cloud` currently names only the second; and MCP-SURFACE 36 already records
  that `download_model` **refuses outright with a remote target configured**, which would make the
  whole curated one-click install local-only and something the Models step has to say rather than
  offer. Deliberately its own session (tasks/phase-5.md T-515).

- **2026-09-06 -- an exported track carries its tags and its cover; the library's copy never does.**
  A cover was app-only: `Track.cover` pointed at an `ArtId` the Library rendered, and `export_track`
  was `std::fs::copy`, so the file a user handed to anyone else had no artwork and no title. Two
  owner decisions shaped **T-516**. **(1) A full tag set, not just the artist**: title, artist, album,
  track number, year and artwork. Only **artist** needed anything new -- the rest already exists
  (`Track.title`, the album list the track sits in and its position there, `Provenance.created_at`,
  `Track.cover`) -- so `config.json` gained one `export.artist` field and a **Release details** card
  on Setup, and nothing else was invented. **(2) Export only.** Send-to hands a path to Latent Mixing
  / Mastering, which are mid-flight; tagging that path is deliberately left to **the version after
  this one**, once that work settles. Two rules are worth keeping: the tags go on **the copy only**,
  because the file under `tracks/` is the artifact its provenance sidecar claims to describe and
  tagging it would silently change what the recipe reproduces; and **tagging never fails an export**
  -- a deleted cover or an unwritable format is a warning beside a file the user still gets, never an
  error instead of one. The album is the **first list containing the track**, in the project's order,
  because an export names a track and nothing in the request says which release is meant -- stable
  rather than arbitrary, which is the property that matters.
...
- **No test feeds a `config.json` without `schema_version`.** Found by a control mutation while
  reviewing T-506c-a: deleting `#[serde(default = "default_schema_version")]` breaks nothing,
  because every fixture in `config.rs`'s tests carries the key. The default is real behaviour a
  user hits on a hand-edited file; it is simply unpinned. One line in an existing test, not worth
  its own task, but worth not rediscovering. (The same run confirmed the `#[serde(default)]` on
  the file's `Option` fields is **stylistic** -- serde already defaults a missing `Option` to
  `None` -- so those attributes carry no guarantee, and a test is the only thing that does.)
...
### 2026-09-01 (later still) -- T-408b-front landed; a project holds many lyric documents

Implemented architect-direct (the reference was in the brief; WORKFLOW 1). A `DocumentPicker` sits
above the brief form in Lyrics Studio: a `<select>` over the project's documents, **New**, and
**Delete document** with an inline confirm and -- on a refusal -- the message at the picker (the
T-408a placement rule). The store gained `docs`, `selectedDocId`, `loadDocs` (replacing the single
`loadDoc` on mount), `selectDoc`, `createDoc`, `deleteDoc`, a `confirmingDocDelete` marker and a
dedicated `deleteDocError`. Frontend **399 -> 405** (six store tests; the old single-doc load test
became the multi-doc one, happy paths named).

**Two design choices worth recording:**
- **`selectDoc` re-opens the document from disk, not from the cached `docs` list.** `lyrics_list`
  returns full documents, but a `commit`/`approve` since the list was fetched saved a newer version
  to disk that the cache would not have. Reading the cache on a switch would silently drop that
  version; `openLyricDoc(id)` always reads the current file. The `docs` list is the picker's index
  only. This also resets the draft on every switch, so an unsaved edit never bleeds between documents.
- **The selection is session-only.** The studio opens the first document each launch; persisting the
  chosen document across restarts (the config-field shape T-401 used for projects) is deferred, not
  built -- noted in the brief's out-of-scope.

Gate green: create-core 174, library 100, mcp-bridge 96, llm-bridge 35, src-tauri 111, **frontend
405**. **Next:** the T-408b producer click-through (five steps at the end of
[the brief](tasks/t-408b-brief.md)) -- multi-document switching, and the document delete refused
against `my-first-song`'s one document (19 tracks on v31). Then T-408c (`delete_album`), T-408d
(project delete).

### 2026-09-01 (later still) -- T-408b click-through passed; T-408c (album delete) landed

The producer reported **T-408b passed all five steps**. Picked up T-408c the same turn.

T-408c is the **pattern-breaker of the T-408 group, and the brief leads with why.** Every other
part deletes a file and refuses when something references it. An album does neither: it is a
**named entry in `project.albums`** (`AlbumList { name, tracks }`, name-addressed since T-403
precisely so it never maps to a path), with no file of its own. Three things follow, all recorded
in the brief so nobody re-adds machinery the shape does not want:
- **No trasher.** The roadmap's "T-408 ... reusing `trash_to_os`" is about the document (b) and the
  project tree (d). An album has nothing on disk to move; injecting a trasher here would be dead code.
- **No refusal, no new `LibraryError`.** a and b refuse because a track points *at* the
  version/document; nothing points *at* an album, so an existing album always deletes.
- **The tracks are never touched.** The one invariant that matters, and the mutation a reviewer
  would most want to break: `delete_album` removes the `AlbumList` and nothing else, leaving every
  id it held in `project.tracks`. `test_delete_album_leaves_its_tracks_in_the_project` is its guard.

Implemented architect-direct (WORKFLOW 1). `library::albums::delete_album` (three tests, **library
100 -> 103**); the `album_delete` command; bridge `deleteAlbum`; the store gained `confirmingDelete`
(store-held so one row confirms at a time, the a/b shape) with `askDelete`/`cancelDelete`/`deleteAlbum`,
which closes the album if the deleted one was open; and an `AlbumDelete` inline confirm beside Rename
whose copy -- "Delete 'X'? Its tracks stay in the library." -- carries the one reassurance a user
needs (frontend **405 -> 410**, five store tests). **Three mutations killed** against a **file-copy
backup** (`remove(0)` for `remove(index)`, the removal no-op'd, the `save_project` dropped -- not
`git checkout`, the T-408a trap).

Gate green: create-core 174, **library 103**, mcp-bridge 96, llm-bridge 35, src-tauri 111, **frontend
410**. **Next:** the T-408c producer click-through (five steps at the end of
[the brief](tasks/t-408c-brief.md)) -- above all, delete an album with tracks and confirm every one
of them is still in the library. Then T-408d (project delete: the whole `projects/<slug>/` tree to
OS trash, exercising `selected_project`'s never-yet-reached "configured slug no longer exists" arm).

### 2026-09-01 (session close) -- T-408c click-through passed; only T-408d remains in T-408

The producer reported **T-408c passed all steps** -- deleting an album (empty and with tracks) left
every one of its tracks in the library, and nothing reached the Recycle Bin (no file was ever
involved). **T-408c is complete.**

**Where T-408 stands:** a (lyric version), b (lyric document) and c (album) are done and
click-through-passed. **Only d (delete a project) remains, and it is not yet briefed.** It is the
one part that most fully exercises the delete-to-trash discipline: the whole `projects/<slug>/` tree
-- tracks, sidecars, lyrics, `project.json` -- moves to OS trash via `trash_to_os`, and it is the
**first thing in the repo that can reach `selected_project`'s "configured slug no longer exists"
arm** (decisions log 2026-08-30), built and tested but never yet exercised. Deleting the *selected*
project must land the app on another project (or the no-project state), not on a dangling slug.

**Starting the next session:** briefs are written one at a time, each after the previous lands, so
the first action is to **write the T-408d brief** (there is no `tasks/t-408d-brief.md` yet) against
the existing `delete_track`/`delete_doc` pattern -- injected trasher, file-first/record-last, id
never reused -- extended to a whole directory tree, then implement architect-direct and mutation-test
the destructive path against a file-copy backup (not `git checkout`, the T-408a trap). After T-408d,
T-408 closes and the phase moves to **T-409** (the song title, carried) then **T-406** (provenance
inspector). Two noted-not-scheduled items still stand: the Library player below-the-fold layout
(docs/CSS-TODO.md) and persisting the selected lyric document across restarts.

Gate green at close: create-core 174, library 103, mcp-bridge 96, llm-bridge 35, src-tauri 111,
frontend 410. Tree clean.

### 2026-09-01 (later still) -- T-408d briefed and landed; T-408 is code-complete end to end

Wrote [tasks/t-408d-brief.md](tasks/t-408d-brief.md) and implemented it architect-direct the same
session. **T-408d takes the delete pattern furthest by needing the least of it.** The earlier parts
trash files *and* edit an outer `project.json`, which is why they carry the file-first/record-last
discipline. A project **is its directory**, so there is no outer record to keep in step
(`list_projects` reads the filesystem) and no id counter to preserve: `delete_project` moves the
whole `projects/<slug>/` tree to the OS trash in **one** `trash` call, and the project lands in the
Recycle Bin as a single restorable folder. Three design points, each pinned by a test that kills the
mutation away from it:

- **Existence is checked on the directory, not the record.** Unlike `delete_track`/`delete_doc`,
  `delete_project` does **not** `load_project` first -- a project whose `project.json` is malformed
  must stay deletable, or the one project the user most wants gone is the one they cannot remove.
  `project_dir` still whitelists the slug. (Mutation: swap the `dir.exists()` check for a
  `load_project` guard -> the malformed-project test fails.)
- **The selection is left alone, on purpose.** Deleting the *selected* project leaves
  `config.default_project_slug` naming a gone slug; both `projectctx::selected_project` and the
  frontend's `effectiveProjectSlug` already resolve that to the first remaining project -- the same
  fallback a hand-edited config hits. **This is the first flow to reach that arm from a real
  deletion** (decisions log 2026-08-30). Reconciling config here would duplicate a resolution the app
  already trusts and couple a filesystem op to the config store, so the command doesn't write config
  and the frontend `deleteProject` re-lists rather than re-selecting. A new `projectctx` test
  produces the condition the real way (delete the configured project, assert `selected_project` lands
  on the sibling), where the pre-existing unit test only *simulated* a gone slug.
- **The confirm names the whole scope.** The most destructive delete gets the strongest copy:
  "Delete 'X'? This trashes the whole project -- every track, lyric and album in it -- to the Recycle
  Bin." Store-held confirm, one row at a time, mirroring a/b/c.

Mutation pass by hand against a **file-copy backup** (not `git checkout` -- the T-408a trap): the
existence guard flipped (killed by the unknown-slug + malformed tests), the guard swapped to
`load_project` (killed by the malformed test), and the return value taken before the trash instead of
after (killed by the remaining-projects test). All three killed. **T-408 is now code-complete across
all four parts;** only the T-408d producer click-through remains before the phase moves on.

**Starting the next session:** if the producer reports the **T-408d click-through** passed
(five steps in the brief -- delete a non-selected project, delete the *selected* project and confirm
the app lands on another with no restart, and the trashed folders whole in the Recycle Bin), mark
T-408d complete and **T-408 closes**; the phase then moves to **T-409** (the song title, carried from
Lyrics Studio to the export filename) then **T-406** (provenance inspector, last). T-409 has no brief
yet -- write it first, one brief at a time. Two noted-not-scheduled items still stand: the Library
player below-the-fold layout (docs/CSS-TODO.md) and persisting the selected lyric document across
restarts.

Gate green: create-core 174, library 109, mcp-bridge 96, llm-bridge 35, src-tauri 112, frontend 414.
Tree clean.

### 2026-09-01 (session close) -- T-408d click-through passed; T-408 is complete, phase moves to T-409

The producer reported **T-408d passed all five steps**: a non-selected project deleted cleanly and
the others stayed; **deleting the *selected* project landed the app on another with no restart** (the
fallback arm reached from a real deletion for the first time, exactly as designed); and the trashed
project folders were whole and restorable in the Recycle Bin, never hard-deleted. **T-408d is
complete, and with it T-408 closes -- all four parts (a lyric version, b lyric document, c album,
d project) are done and click-through-passed.** The app can now delete every kind of content it
makes, each to the OS trash (except the album, which has no file), each refusing or reassuring where
...
already have the field; only the UI is missing); (2) **`GenerationSpec` gains `title: Option<String>`**,
prefilled in the Audio Studio from the selected lyric document and editable there -- resolving it at
ingest from `spec.lyrics.doc_id` is **rejected on evidence** (one of the producer's 20 tracks has no
lyric ref, so ingest would have no title source, and provenance should record what the user *chose*);
(3) `ingest.rs` sets `Track.title` from the spec; (4) **export offers the title as the default
filename, sanitised before the dialog** (a `/`/`:`/trailing-dot title is legal in a doc, illegal as a
Windows filename); (5) the Library shows it, id fallback kept. Traps: the **audio file keeps its id
name** (`tracks/tr-0007.flac`, never `Midnight.flac` -- title is display-and-export only, ARCHITECTURE
8); `Track.title` is a **snapshot, not a link** (renaming the doc later must not retitle existing
tracks); and the new `GenerationSpec` field is an **ARCHITECTURE 5/7 interface change**, so that doc
edit lands in the same commit as the code. It will likely **split into lanes** the way T-404/T-405
did (schema+ingest / studio prefill / export+display). **T-406 (provenance inspector)** is the last
task, then Phase 4 closes. Two noted-not-scheduled items still stand: the Library player below-the-fold layout
(docs/CSS-TODO.md) and persisting the selected lyric document across restarts.
...
### 2026-09-02 -- T-406 briefed and split; lane a (the inspector) landed

Wrote [tasks/t-406-brief.md](tasks/t-406-brief.md) and landed lane a architect-direct. Verifying the
surface first settled the shape: **T-406 is entirely frontend.** `listTracks()` already returns the
full sidecar (`provenance.spec` with every input, the LoRA stack, the lyric ref, the title;
`resolved_slots`; `comfy`; `template`), and the library store was **discarding** all of it in
`trackRows`, keeping only the flattened display `TrackRow`. No Rust, no bridge change.

**Lane a -- the read-only inspector.** The store now keeps `byId: Record<string, Track>` (the raw
tracks, built in `load` beside `tracks`), a pure `provenanceView(track)` builds the display as omit-
when-empty sections (Inputs / Lyrics / Resolved slots / Server), and a **Details** disclosure on each
`TrackCard` renders them (local open/closed state -- a read-only toggle needs no cross-row exclusivity,
unlike the store-held delete/rename confirms). `formatValue` stringifies a tagged `InputValue` by its
`.value`, **never `String(v)`**, which would print `[object Object]` for the `{type,value}` wrapper --
the one real trap in the display, and the thing a test pins. Long slot addresses and LoRA paths wrap
inside an `overflow-x:auto` body, so the page never scrolls sideways (the T-407 rule). Four tests:
`byId` carries the raw track through `load`; `provenanceView` renders a seed as its number, builds
every section from a full sidecar, and returns `[]` for a bare older sidecar (no inputs, ref, slots or
comfy). frontend 426 -> 430.

**Lane b remains: "re-use these settings"** -- the inverse of `specFor`. `controlValues` reverses
`specInputs`, `stackFromLoras` reverses `specLoras`, `paramPanel.hydrate` / `loraPanel.hydrate` load a
past spec into the panels, a `reuse(spec)` orchestrator on the generatePanel store carries the title
override and navigates to the Audio Studio, and a "Re-use these settings" button lives in the Details
panel. **The one behavioural trap:** re-use lands with `seedPinned: true` (the opposite of a fresh
Generate, which re-rolls, T-316), so the reproduced track uses the sidecar's exact seed -- the test
asserts the loaded seed is verbatim and pinned. When lane b's click-through passes, **Phase 4 closes.**

Gate green: create-core 175, library 109, mcp-bridge 96, llm-bridge 35, src-tauri 114, frontend 430.
Tree clean.

### 2026-09-02 -- T-406 lane b: "re-use these settings"; T-406 code-complete

Landed lane b architect-direct, frontend-only -- the inverse of `specFor`. Two pure reversers in
`state/generate.ts`: `controlValues` unwraps a spec's tagged `InputValue`s back to panel values (a bool
input is skipped -- the panel has no boolean control, `specInputs` emits none), and `stackFromLoras`
turns `LoraRef`s back into `StackRow`s (label derived from the file stem). Two `hydrate` actions load a
past spec into the panels: `paramPanel.hydrate` merges the spec's values over the profile's defaults
and -- **the one behavioural trap** -- sets `seedPinned: true`, so the next Generate reproduces the
track rather than re-rolling the seed (T-316's opposite); `loraPanel.hydrate` loads the catalog and
sets the stack (a re-used LoRA the catalog no longer offers stays, flagged by `missingFrom`). Both
overwrite even when the same profile is already selected, unlike `load`. A `reuse(spec)` orchestrator
on the generatePanel store calls both, sets the Audio Studio title override from `spec.title`
(T-409), and navigates to the Audio Studio; a **Re-use these settings** button in the inspector's
Details wires it.

Five tests: `controlValues`/`stackFromLoras` invert `specInputs`/`specLoras`; `reuse` loads the spec
with the seed **verbatim and pinned**, the stack, the title, and navigates; a `submit` right after
`reuse` sends the reused seed (not a fresh one); and a `reuse` of a spec whose profile was removed
surfaces the panel's "no profile answers to …" error, not a blank form. The seed-pin trap was
mutation-checked by hand (file-copy backup): flipping `hydrate` to `seedPinned: false` is killed by
both the pin assertion and the submit-after-reuse verbatim-seed test. `tsc -b` caught the bool-value
type gap in `controlValues`, now handled. frontend 430 -> 435.

**T-406 is code-complete across both lanes.** End to end: open a track's Details to read its full
recipe, click Re-use these settings, and the Audio Studio opens with the profile, inputs, LoRA stack,
title and **exact seed** loaded -- Generate reproduces the track. **Its producer click-through is
pending** (five steps in [tasks/t-406-brief.md](tasks/t-406-brief.md), the key one being that the
re-used seed is the sidecar's, not a fresh roll). **When it passes, T-406 is done and Phase 4 closes**
-- every milestone line and both owner-decision additions (delete-for-everything, the carried title,
the inspector) complete. The two noted-not-scheduled items would remain for a later phase: the Library
player below-the-fold layout (docs/CSS-TODO.md) and persisting the selected lyric document across
restarts.

Gate green: create-core 175, library 109, mcp-bridge 96, llm-bridge 35, src-tauri 114, frontend 435.
Tree clean.

### 2026-09-02 (phase close) -- T-406 click-through passed; Phase 4 is COMPLETE

The producer reported **T-406 passed all steps**: a track's Details showed its full recipe (inputs,
resolved slots, lyric ref, ComfyUI version/url), a titled track read cleanly (slots as `address: value`,
not `[object Object]`), **Re-use these settings** opened the Audio Studio with the profile, inputs,
LoRA stack, title and **the same seed** loaded, a Generate from there reproduced the track (seed not
re-rolled), and re-using a removed profile showed the panel's error rather than a blank form. **T-406
is complete, and with it Phase 4 (Library & Player) closes.**

**What Phase 4 delivered** (all click-through-passed): projects became first-class (T-401); playback
with an AnalyserNode visualizer (T-402); album lists (T-403); Send-to link-outs (T-404); track actions
-- delete-to-trash, rename, export, reveal (T-405); shared scrollbar styling (T-407, debt pulled
forward); **delete for every kind of created content** -- lyric versions, documents, albums, projects
(T-408, four parts); **the song title carried** from Lyrics Studio through the spec to the track, the
Library and the sanitised export filename (T-409, three lanes); and **the provenance inspector +
"re-use these settings"** (T-406, two lanes). The milestone check was met at T-405; T-406/T-408/T-409
were the owner-decision additions that finished the phase.

**Phase 4 is not git-tagged.** Phases 0-2 carry `phaseN-done` tags, but Phase 3 was closed
2026-08-30 with a docs-only close and no tag -- the recent precedent -- so this close matches it.
Tagging `phase4-done` (and, if wanted, a retroactive `phase3-done`) is a one-command step left to the
owner; say the word.

**Starting the next session: open Phase 5** (cover art, polish, packaging -- T-501...). There is **no
`tasks/phase-5.md` yet** and **no phase-start check has been done.** Per the session ritual, the first
work is the phase-start check (verify the image/cover-art surface the way each phase has re-verified
its third-party surfaces -- comfy-mcp for image generation here -- before writing any T-brief), then
open `tasks/phase-5.md` from the ROADMAP scope: a cover-art view over the image profile, a first-run
polish pass, an empty/degraded-states audit, installer builds (Windows NSIS/MSIX first, then macOS dmg
+ Linux AppImage via CI), THIRD-PARTY-LICENSES generation, and public-repo readiness (CONTRIBUTING,
issue templates). Milestone: an installable build on a machine that never had the dev toolchain. Two
noted-not-scheduled Phase 4 items carry forward as backlog: the Library player below-the-fold layout
(docs/CSS-TODO.md) and persisting the selected lyric document across restarts.

Gate green at phase close: create-core 175, library 109, mcp-bridge 96, llm-bridge 35, src-tauri 114,
frontend 435. Tree clean.

### 2026-09-02 (later) -- Phase 5 opened; model catalog scoped from a live surface re-verification

The owner opened Phase 5 by flagging four things before any build. Grounded each against the code
rather than memory, ran the phase-start check live, and wrote the design -- **docs first, build
order deferred** (owner's chosen sequence).

**The four flags, verified:**
1. **Lyrics doc-picker floats without a card** -- `DocumentPicker` is a bare `<div class="doc-picker">`
   ([app/src/views/LyricsStudio.tsx:300](app/src/views/LyricsStudio.tsx)) with only `margin-bottom`;
   every sibling block is a `panel`. At full width its actions push to the far side. -> **T-501**.
2. **The model catalog was dropped from the plan** -- see the decisions-log entry above. It was the
   point of the Setup page beyond LLM selection, survived as a mis-stated half-line in ARCHITECTURE
   §10, and was never built. -> designed in **ARCHITECTURE §10a**, tasks **T-504/T-505**.
3. **The Library player sits off-screen** -- `<Player/>` is the last in-flow element
   ([app/src/views/Library.tsx:122](app/src/views/Library.tsx)); `.player` is not sticky/fixed.
   Already noted as backlog at the Phase 4 close (CSS-TODO). -> **T-502**.
4. **The visualizer blurs in full screen** -- canvas backing store hard-coded `640x120`
   ([app/src/components/Visualizer.tsx:91](app/src/components/Visualizer.tsx)) while CSS stretches it
   to `width:100%`; the bitmap upscales. -> **T-503** (DPR-sized backing store + a visual-quality pass).

**Phase-start check (live), recorded in [docs/MCP-SURFACE.md §32](docs/MCP-SURFACE.md):** ComfyUI up
(RTX 5060 Ti, comfy-cli 1.16.0, core v0.34.2 current). The headline finding: **comfy-mcp has no
model-hub search.** The browsable catalog surface is the built-in **template gallery**
(`search_templates`) -- each row a *workflow + model* bundle -- and it is the same gallery Phases 1-3
used for audio, so **one surface serves both audio and image** (a `flux` query returned 13 local
image templates). `search_models` is local-readiness only; `download_model` needs a known URL;
`list_partner_models` is a paid cloud tier.

**Design written:** ARCHITECTURE §10 corrected + **§10a** (the catalog rides the gallery, filtered
`api:false`, split by `output_type`, adopting via the T-313 import-to-profile path; local-only for
v1). ROADMAP Phase 5 line expanded, paid cloud tier moved to Future. **tasks/phase-5.md opened** with
the phase-start record, owner decisions, a "what already exists" audit, and a scoped task map
...
### 2026-09-02 (later still) -- T-504 landed: the catalog gallery browse + readiness seam

Ran Aider on the T-504 brief; the working tree was **the brief's three files and nothing else**.
Review: `browse_templates` sends `type`+`exclude_api:true`+`offset` (query omitted when `None`); the
two commands mirror the models-step structure (`ensure_connected`/`EnsureError`, never `Err` for a
service state -- `catalog_readiness` returns `LocalCheck::Unknown`, not an error, when no comparison
was made); `create-core` untouched (verdict derives in the T-505 store); the models step still reads
`local_check` only in its doc comment (the MiniMax guard holds). One rustfmt line-wrap in the live
test was the only gate miss -- `cargo fmt` fixed it. Gate green; **mcp-bridge 96->98, app 114->117**,
...
### 2026-09-02 (later still) -- T-505a landed: the catalog browse/readiness store

Ran Aider on the T-505a brief; the tree was **the brief's three new files and nothing else**, matched
closely. Review: `bridge/catalog.ts` mirrors the T-504 types + two invoke wrappers; `verdictFor` maps
`runnable:true`->ready, `runnable:false`->not_ready carrying `errors` **verbatim** (summary fallback
when empty, non-string errors coerced), `{state:'unknown'}`->unknown; `rowViewFor` never labels an
uncheckable row "Not installed"; the store guards on `isTauri`, sends `undefined` for an empty query,
...
### 2026-09-02 (later still) -- T-505b click-through passed; a Models-step fix-up

**T-505b click-through passed** -- the catalog toggle, the 163-row image gallery, search, and the
readiness pills all worked, and a stopped ComfyUI read "Can't check" (the catalog's own stopped-state
handling is correct). The click-through surfaced one defect, **pre-existing and outside the catalog**:
the **Models step** (not the catalog) rendered the raw `server_not_running` tool diagnostic -- ~300
characters of `urlopen`/`WinError` prose ending in "check `--where` and network connectivity" -- when
ComfyUI is stopped. Same ugliness class T-315 fixed for the job path. Fixed same-session
(architect-direct): a new `models::inventory_detail` helper returns `None` for `server_not_running`,
so the frontend falls back to its own clean "Start ComfyUI above." line; any *other* (genuinely
unexpected) error keeps its detail, where the diagnostic is the only clue. One test; **app 117->118**,
gate green. The catalog itself was untouched. Next: **T-505c** (curated one-click install on a catalog
row).

### 2026-09-02 (later still) -- T-505c briefed: curated one-click install on a catalog row

Briefed [T-505c](tasks/t-505c-brief.md). The design fell out of two live checks + a repo read. **The
join:** a shipped profile already carries `comfy.template` (the gallery template it rides), and those
strings are the **exact** `name` the catalog's browse returns -- verified live against the running
gallery: `audio_ace_step1_5_xl_turbo` and `audio_minimax_music_3` come back as those literal names.
So a catalog row is "curated" iff a loaded profile's `template` equals its `name`. **The reuse:** the
Models step's `ModelRow` already does the whole install UX from the singleton `useModelsStore`
(`install`/`installView`/`rowFor`), so a curated catalog row reuses it verbatim -- an install from the
catalog *is* the Models step's install, one transfer, both surfaces reflecting it. No new Tauri
command, no new install code. **The one correctness rule carried forward:** a curated row's readiness
is the profile's, never `local_check` -- MiniMax reads Installed despite its template's fp16/int8
`runnable:false` (the MiniMax lesson). The only backend change is one field, `ProfileStatus.template`
(Rust + TS mirror), the join key; the join itself is a pure, tested TS helper `curatedIndex(view)`.
Bare rows keep T-505b's `local_check` path untouched. Seven files, has a click-through. Docs/brief
only; gate unaffected.

### 2026-09-02 (later still) -- T-505c landed (awaiting click-through); a rules-of-hooks fix

Ran Aider on T-505c; the five non-component files matched the brief exactly -- the `template` field
(Rust + TS mirror), the pure `curatedIndex` join, the `refresh` busy-guard, and both test files
(three `curatedIndex` cases + the dedupe-race and busy-guard store tests). **Review caught a
rules-of-hooks violation the gate could not** -- and it was **my error in the brief**: I specified a
`CatalogRow` that early-returns `<CuratedRow>` *before* calling the bare-row hooks
(`useCatalogStore`/`useRef`/`useEffect`). The catalog fires two independent async loads (the gallery
browse and the models view); whichever order they resolve in, a row that starts bare and turns
curated when the view lands would flip the same instance from three hooks to zero -- React throws
*"Rendered fewer hooks than expected"*. Race-dependent, so `tsc`/oxlint/vitest all pass and it
detonates only in the click-through. Fixed by branching in the parent `.map` between two sibling
components (`BareRow` / `CuratedRow`), each of which always runs its own hooks; a row turning curated
is now a clean remount (same key, different type), never a hook-order change. Adding a **required**
`ProfileStatus.template` field also broke an untouched fixture (`state/profiles.test.ts`) -- an
eighth file, necessary fallout, one `template: null` line. **src-tauri 118->119, frontend 448->453**;
...
### 2026-09-02 (later still) -- T-505d-a landed: the adopt backend seam

Ran Aider on T-505d-a; the diff matched the brief -- `catalog_adopt_begin(name)` (ensure-connected →
`fetch_template` to a temp → the factored `adopt_from_fetched` → `import_into`, temp removed on every
path), the bridge mirror, the registration, two mock tests (import + cleanup, refusal + cleanup) and
an `#[ignore]` live test. Review found **no defects** -- the reuse held, `import.rs` is untouched, and
the deref from the `Arc<LocalComfy>` `ensure_connected` returns to `import_into`'s `&LocalComfy`
compiled clean. Two gate nits, both from **my brief's own test code**: a rustfmt line-wrap and a
clippy `len() >= 1` → `!is_empty()`; fixed. Aider also made one unrequested cosmetic doc edit (a module
...
frontend 453->459; full gate green.

### 2026-09-03 (end of session) -- T-505d-d click-through passed; adopt works end to end

The producer ran it. **Passed**, and I verified it independently against the emitted profile on disk
(`%APPDATA%/com.latentbeats.create/profiles/flux-2-klein-9b-text-to-image.json`) rather than taking
the screen's word for it. That one file confirms all three lanes at once:

- `kind: image`, `output: {save_node: "SaveImage", prefer_lossless: false}` -- **T-505d-b**.
- `tags -> 75/74.text` (the positive encoder) and `negative -> 75/67.text` (the negative one), plus
  seed/steps/cfg on their subgraph addresses -- **T-505d-c**, exactly as designed.
- `id: flux-2-klein-9b-text-to-image`, from the gallery title and not the adopt temp file -- the
  **T-505d-d** headline. `template: None`, `workflow:` the stored copy.

Two findings, one real and one not:

**Not a defect.** The Models step shows the new profile as "Cannot check" / "This profile does not
list the model files it needs", with the licence line "Not declared (imported workflow)". Both are
pre-existing and deliberate: the licence string is `emit.rs`'s (the app cannot know what someone's
own graph is licensed under), and the readiness is `ProfileReadiness::Undeclared` because `emit` sets
`models: []` for **every** imported profile and has since T-313 -- not a regression from this chain.
Checked every consumer of readiness (`generate.ts`, `generatePanel.ts`, `GenerateBar.tsx`,
`generate.rs`, `profiles.ts`): **nothing gates on it**, so the profile is selectable and generable and
T-506 is not blocked. Logged against **T-507** with the concrete fix (declare `comfy.models` from the
graph's loader COMBO slots, folders via `search_models`, leaving `source_url`/`size_bytes` `None`).

**A real visual defect, fixed.** The row's action button used the wizard's `.setup-actions` /
`.setup-button`: no top margin -- and `.catalog-row` is a plain block where every other child sets its
own -- plus step-sized padding, so it sat oversized and flush against the tag chips. Added
`.catalog-row-actions` with a compact button override. **The same bug was sitting in `CuratedRow`'s
Install button and had never been seen**, because T-505c's click-through found nothing missing so
that button never rendered; both rows now use the new class. I could not verify this one visually --
the app is Tauri-gated so the preview pane cannot run it, and a static CSS harness only rendered as a
stale snapshot under the pane's CSP -- so **the button's appearance still wants the producer's eye**,
even though the reasoning and the gate are sound.

Phase 5's adopt chain (T-505d a-d) is complete. Next: **T-506, cover art generation** over this
profile -- the largest single feature left, and expected to split.

### 2026-09-03 (later) -- T-506 opened: the image pipeline run end to end, outside the app

Session ritual first: PROJECT.md, ARCHITECTURE.md and the phase file all agree with `git log` since
the last entry -- no drift to fix, the T-505d chain is recorded exactly as it landed.

Then scoped T-506 (cover art) the way this project is supposed to: **not by reading our own code and
reasoning about it, but by running the thing**. The adopted Klein profile's stored workflow copy was
put through the same sequence `build_and_submit` performs -- `set_workflow_slot` -> `validate` ->
`run` -> `fetch_outputs` -- and then `GET /history/<prompt_id>` was read, because that is the only
surface that says what actually executed. Full write-up: **docs/MCP-SURFACE.md 35**. It produced a
real 768x768 cover in 22 s and changed the shape of the task three times over.

**The Rust generation half needs no change at all.** `ensure_lossless_output` is a clean no-op at
`prefer_lossless: false` (what T-505d-b emits for an image graph), the LoRA splice is skipped for a
profile with no `loras` block, `validate_workflow` passed, and the prompt, negative, seed and steps
writes **all reached the engine** -- confirming T-505d-c's mapping and 34.4's boundary-fed rule live
on an image graph rather than by analogy with MiniMax. `build_and_submit` is reusable verbatim.

**So the work is entirely on the far side of the output, and one part of it is silent.** `ingest.rs`
files an output only when its extension is in `AUDIO_EXTS` and returns `None` for anything else
**without an error** -- an image job run through the app as it stands completes, reports Done, and
saves nothing, anywhere. There is no artwork record, and `Project` has no art list. That is why the
first lanes are storage and ingest rather than generation, and why this is a task and not a detail.

**The size trap, found before it was written.** Klein exposes four size-shaped slots and **all four
are inert**: `75/62.width|height` (`Flux2Scheduler`) and `75/66.width|height` (`EmptyFlux2LatentImage`)
are link-fed from interior `PrimitiveInt`s, and the effective addresses are `75/68.value` /
`75/69.value`. Proved rather than argued, in one run: 1536 written to the obvious addresses and 768
to the primitives, and the PNG on disk is **768x768**. This is 18.1 again -- ACE-Step's seed, one
model over -- and `audit_slots` already classifies it correctly with no change, so the pipeline would
have refused a naive size profile rather than recording a size the engine ignored. The consequence is
a planning one: **image size stays the graph's own for v1** (1024 square on Klein, which is the right
shape for a cover anyway), and a size control is deferred **with its evidence** as a role-suggestion
problem of the T-505d-c kind, not a UI one. Owner's call whether it joins this phase.

T-506 is now five lanes (phase-5.md): **a** the artwork record + storage, **b** `generate_image` +
art ingest, **c** the art generation store, **d** the CoverArt view -- the lane with the
click-through -- and **e** attach-to-track/album plus `delete_art`. Two decisions taken while
splitting: `Provenance` is **reused verbatim** for artwork (it already records a generated *asset*,
none of which is audio), and a cover reference belongs in the **track sidecar**, not on the artwork,
per ARCHITECTURE 8's one-source-of-truth rule. A third is named for lane c: `paramPanel.ts` is a
module-level singleton, so a CoverArt view loading an image profile into it would reset the Audio
Studio's values on every view switch -- the T-505d-d singleton lesson, one store over.

**T-506a is briefed** ([tasks/t-506a-brief.md](tasks/t-506a-brief.md)) in the Aider lane -- it is
mechanical mirroring of `tracks.rs` across two crates, which is what the executor is for. Its
fixture is frozen ahead of the run the way Klein's was for T-505d-c: `testdata/images/klein-cover.png.head`
is the first 33 bytes of the cover this session generated, so the PNG header parser is tested
against a real file rather than a hand-built one.

### 2026-09-03 (last) -- T-506a landed: a generated image has somewhere to go

The artwork half of the library exists. `ArtId`, `Artwork` (reusing `Provenance` **verbatim** --
it records a generated *asset*, and nothing in it is audio-specific), `create-core::image` with a
hand-rolled PNG IHDR reader the way `audio.rs` reads a FLAC header, and `library::art` mirroring
`tracks.rs` function for function. `Project` gains `art` and `next_art_seq`, both serde-defaulted,
so every project on disk still loads. ARCHITECTURE 8 updated in the same commit.

Two things the executor did that the **brief had wrong**, and it was right both times:
`#[serde(transparent)]` on `ArtId` (the brief's reference code dropped it; `TrackId` has it, and
without it every stored id would have been an object) and `pub mod art;` **after** `albums` rather
than the "before `albums`" the brief asked for, which is where alphabetical order actually puts it.

Three defects fixed in review, the first two of which mean **the gate was not run before the diff
came back**:

- **`pub use albums::AlbumSet;`** -- an unrequested re-export of a type that does not exist.
  `albums.rs` has only functions. A compile error.
- **Two test-only imports at module scope.** `art.rs` copied `tracks.rs`'s import line, but
  `tracks.rs` needs `load_project`/`save_project` for `delete_track`, which this lane does not
  have; only its tests use them. `-D warnings` refuses that, correctly.
- **A vacuous test.** `test_artwork_sidecar_roundtrips` built a fully populated `Artwork`,
  serialised it, read it straight back and compared -- which cannot fail unless a derive is
  removed, and duplicated the round-trip `library` already had. Rewritten to assert the invariant
  the three `#[serde(default)]`s exist for: the keys are stripped from the JSON and the sidecar
  must still load with `None` for each and its provenance intact. This is WORKFLOW 4.2's question
  applied literally -- would it fail if the thing it guards were broken?

**Eight mutations, eight killed**, against the guards a wrong one would be found by a user rather
than the suite: the zero-dimension check, the PNG magic and IHDR markers, the id length bound, the
missing-sidecar warning, the id counter's advance, the `..` escape check, and -- the one worth
naming -- `#[serde(default = "default_art_seq")]` weakened to `#[serde(default)]`, which would
default the counter to 0 and mint `ar-0000`.

create-core 184 -> **191**, library 109 -> **124**; gate green. Next: **T-506b**, `generate_image`
plus the art ingest, which is where the silence found in 35.4 gets closed.

### 2026-09-03 (last) -- T-506b landed: the silent PNG drop is closed

An image generation now ends where an audio one does. `PendingTrack` became `PendingOutput`
carrying a `ModelKind`, and **the record decides which ingest runs, never the file extension**;
`ingest_outputs` returns a `Saved` per asset so the pump emits `track://saved` or `art://saved`;
the download directory is chosen by kind; and a pure `kind_error` refuses a profile queued by the
wrong command **before `ensure_connected`**, so a mismatch never starts a `comfy-mcp`.
`build_and_submit` was not touched -- the live scoping run had already shown it needs nothing for
an image profile. ARCHITECTURE 7 gained the shared-pipeline step in the same commit.

**The first defect was mine.** The brief's `queue_generation` reference signature took
`&ComfyState`/`&ConfigDir` where `ensure_connected` wants the `State` wrappers; the executor
transcribed it faithfully and it did not compile. That is the WORKFLOW 3 rule about reference code
biting on a signature I wrote from memory instead of from the callee -- `comfy.rs` was one grep
away.

Three test defects, all fixed directly:

- **Two kind-guard tests were named for the opposite of what they assert** --
  `kind_error(&ace(), ModelKind::Image)` is a *music* profile on the *image* command, and the test
  called it the reverse. A test whose name contradicts its body is a trap for whoever reads it
  next. One of them also expected the fixture's **filename** (`flux2-klein-9b-image`) where the
  profile's **id** (`flux-2-klein-9b-text-to-image`) belongs.
- **The pipeline test used the input name `prompt`** where the profile declares `tags`, so it
  failed at `resolve_slots` rather than exercising the copy-not-fetch branch it exists for.
- **The title test ingested the same file twice.** Ingest **moves** the output into place, so the
  second run read a path that was no longer there -- a real property of the code the test was
  written against, and worth the comment it now carries.

**Seven mutations, seven killed** -- but one only after rewriting a test. The art counter test
minted and wrote **by hand**, in the order the test chose, so moving the counter save *after* the
file write in the production path left it green. It is now the art mirror of the track burn test,
driven through `ingest_outputs` with a download that is not there. The repo's own doc comment on
that track test already said a hand-ordered test cannot see a reordering; the lesson was written
down and not applied one function over.

Also consolidated in review: `audio_extension` and `image_extension` differed only in the constant
they consulted, and a filter fixed in one copy is a filter the other kind silently does not get.

**Two production lines have no unit test and are click-through items for T-506d**, said plainly
rather than papered over: the download-directory choice and the two-event dispatch both live in
`ingest_if_pending`, which needs an `AppHandle` no test in this crate builds. Every decision either
one makes is derived from the pending record's kind, which *is* tested; what is unproven is the
wiring. **Both proved 2026-09-03** by T-506d's click-through, steps 5 and 6.

One narrow silence survives **by design**: an image job whose outputs are all non-images files
nothing and says nothing, the mirror of the audio path skipping ComfyUI's preview PNGs. Making it
loud would fire on every audio generation that emits a preview, which is most of them.

src-tauri 121 -> **133**; gate green. Next: **T-506c**, the art generation store -- where
`paramPanel.ts` stops being a singleton.

### 2026-09-03 (last) -- T-506c split; c-a landed: Cover Art can remember a model and read its work

T-506c was one lane on paper and two in fact: the stores need a config field and two read commands
that do not exist, and putting Rust in the store lane is what the T-504 -> T-505a cadence exists to
avoid. Split, and **c-a landed architect-direct** -- writing its reference *was* writing it, which
WORKFLOW 1 says not to send out.

`Config.default_image_profile_id` is its own field rather than a reuse of `default_profile_id`,
for two reasons. The studios choose independently, so one field would make picking an image model
in Cover Art change the model the Audio Studio generates with. And **the image side has no default
to fall back to**: `ace-step-1.5-turbo` ships, but the app ships no image profile at all -- the only
way to have one is to adopt it from the catalog. So `None` stays `None`, and the view will say
there is no image model yet rather than generating with one the user never chose.

`src-tauri/src/art.rs` is `tracks.rs`'s two read commands with the nouns changed. Checked rather
than assumed while briefing: the asset-protocol scope is already `$APPCONFIG/projects/**`, so
`projects/<slug>/art/` needs no change -- worth recording so the view lane does not go looking.

**The brief's file list was short by five, and the tripwire is what found it.** Adding a required
field to a type mirrored in TypeScript touches every test that builds a `Config` literal -- five of
them. `test_wire_fixture_matches_current_types` failed first, exactly as designed, and its own
message says the TypeScript types must change in the same commit. This is the first time that
tripwire has fired for a new field since T-005 built it, and it worked.

**Two mutations, both equivalent, and saying so is the point.** Removing `#[serde(default)]` from
the new field changes nothing -- serde already defaults a missing `Option` to `None`, so that
attribute is style, not guarantee. A control mutation confirmed the mechanism rather than assuming
it: the same deletion on the **non-`Option`** `schema_version` also survived, because no test in
the file feeds a config missing that key. That is a real (pre-existing) gap and is now in the
backlog. What the new tests do pin is the behaviour that matters: an old config loads clean with
`None`, and the two profile choices survive a save/load with their own values -- the test a
copy-paste writing one field into the other would fail.

library 124 -> **126**; gate green. Next: **T-506c-b**, the frontend stores, where `paramPanel.ts`
stops being a singleton.

### 2026-09-03 (last) -- T-506c-b landed: the panel stops being a singleton

`paramPanel.ts` is now `createParamPanelStore()` with two instances, and the store body did not
change by a line -- so every existing importer of `useParamPanelStore` kept working and the lane
landed safely with no view to show for it. `state/artGenerate.ts` is the Audio Studio's `submit`
with four differences and their reasons in comments: the art panel, an empty LoRA stack and no
lyric document (an adopted image profile declares neither, so `specsFor` is reused rather than
forked), the title field as the only title source, and `generateImage`. Everything else is verbatim
because each line is a rule already paid for -- `register` inside the loop, sequential `await`, the
seed written back with `setSeed` so it is not pinned.

**Two defects, one of them a deletion.** The executor **overwrote `pickable`** -- an existing
export the brief never mentioned -- by replacing it in place with `effectiveImageProfileId`, which
broke `AudioStudio.tsx`. `tsc` caught it immediately, and the irony is that `pickable`'s own doc
comment says it takes a `kind` parameter *because CoverArt will want the same list filtered the
other way*: the lane deleted the function written for it. Restored verbatim. The second was a seed
test stubbing `global.crypto`, which is getter-only in this Node -- and which the test did not need
at all, since every assertion is about whether the seed **changed** and whether the spec carries
the one on screen, both of which the real generator answers.

**Nine mutations, nine killed, and one of the nine had to be re-run to mean anything.** The first
parallel-submit mutation wrapped the specs in a `Promise.all` that did not actually parallelise the
`generateImage` calls, so its "killed" was not evidence about the sequential-await invariant. Redone
with a real `Promise.all` over the calls themselves, and the concurrency test does catch it. The
other eight: the two panels made one store, the art store reading the audio panel, the missing
`register`, `setSeed` swapped for `setValue` (which would pin a seed the user never chose), the
title dropped, `effectiveImageProfileId` falling back to the audio default, and the busy guard
removed.

frontend 459 -> **476**; gate green. Next: **T-506c-c**, the artwork listing store.

### 2026-09-03 (last) -- T-506c-c briefed: the gallery gets its list

The artwork listing store, mirroring what the Library does for tracks: `bridge/art.ts` over
`library_art` and `art_image_path`, `state/art.ts` holding rows and an asset URL per artwork, and
the `art://saved` subscription so a finished cover appears without a reload.

Three decisions worth the brief's space. The URLs resolve with **`Promise.all` and a per-artwork
catch** -- the sequential-`await` rule `artGenerate.ts` carries is about the one stdio transport to
comfy-mcp and about `submittedAt` ordering the queue, and neither applies to a path lookup; the
catch is there because one hand-edited sidecar must not blank the gallery, which is the same rule
`list_art` already follows on the Rust side by returning a warning instead of an error. The URL map
is **not cached across loads**, because art ids are per-project -- every project has an `ar-0001` --
so a map keyed by id and kept across a switch would render the previous project's cover under the
new project's provenance. And for the same reason `select`/`deleteProject` in `state/projects.ts`
now reload the artwork beside the tracks; the comment there already says the track list belongs to
the selected project, and artwork belongs to it identically.

The lane also **extracts `modelLabel`/`seedText`/`createdDate`/`provenanceView` into
`state/provenance.ts`**. `library.ts`'s own comment calls `queue.ts`'s `modelName` a deliberate
twin; a third copy is where the absent-versus-empty rule drifts, and it would drift in the record a
user reads to prove where a file came from. `provenanceView` takes a `Provenance` after the move
rather than a `Track`, which is also how T-506d gets an artwork inspector without a second one being
written. The extraction is verbatim -- if a moved test needs its assertion edited, the move was
wrong, and that is an acceptance criterion.

### 2026-09-03 (last) -- T-506c-c landed: the gallery has its list

`bridge/art.ts` and `state/art.ts`, mirroring what the Library does for tracks: rows over
`library_art`, an asset URL per artwork over `art_image_path`, `byId` for the inspector T-506d will
want, and the `art://saved` subscription. `state/projects.ts` reloads the artwork beside the tracks
on a project switch and a project delete -- without it the tiles would not merely be stale, they
would render real images from the project the user had just left.

The extraction landed faithfully: `modelLabel`/`seedText`/`createdDate`/`provenanceView` moved to
`state/provenance.ts` keyed on `Provenance`, every moved test kept its assertions, and `trackRows`
produces exactly what it did before.

**The gate was not run before the hand-off** -- two tests failed and four `tsc` errors stood. One of
the two failures is worth writing down, because it is the shape that passes for the wrong reason
when the default happens to line up: `makeArtwork` built its overrides with `??`, so
`width: null` was swallowed and re-defaulted to 768, and the missing-size case the test claimed to
check was never constructed. `library.test.ts` uses the same `??` idiom safely, because there the
default *is* `null`; here the default was a number, and the helper quietly deleted the case. Now
`=== undefined`, with the reason in a comment. The second was a `startListening` test asserting one
subscription while `isTauri` was mocked to a fixed `false` -- the guard refuses before subscribing,
so the assertion was about a call that could never happen. Made togglable, the idiom
`projects.test.ts` already uses, and a second test added for the invariant underneath: outside Tauri
the store must neither subscribe **nor arm `listening`**, or a store that started in a browser could
never listen afterwards.

**Fourteen mutations, fourteen killed** -- one of them only after closing the gap it found.
`modelLabel` dropping its `.trim()` survived: a hole carried in unchanged from `trackModel`, which
mattered more after the move because two stores share the function now, and an emitted profile takes
its display name from a workflow filename, so `'   '` is a value that really arrives. Pinned. The
other thirteen: the size formatter's `&&` loosened to `||` (a tile reading `768 x null`), the title
fallback, the per-artwork catch removed so one bad sidecar rejects the whole gallery, **the URL map
cached across loads** (the cross-project bug the brief refused, and the anti-cache test does catch
it), the empty-warning sentence, `byId` left empty, `listening` armed before the guard, the guard
dropped, `undefined` instead of `null` for an unresolved URL, both `projects.ts` reloads, `seedText`
stringifying the wrapper, and a control on `trackRows` to confirm the library still guards its own
rows after the extraction.

frontend 476 -> **497**; gate green. Next: **T-506d**, the CoverArt view -- **the lane with the
click-through**, and where the two untested lines in `ingest_if_pending` finally get proven.

### 2026-09-03 (last) -- T-506d briefed: the Cover Art view, and the click-through that proves the ingest

The last lane before cover art works end to end. Picker, panel, Generate, the shared queue, and a
grid of what has been made -- plus two extractions that stop it becoming a second Audio Studio.
`ParamPanel` takes its store as a prop, which is the last thing pinning T-506c-b's factory to a
single instance; and `ProfilePickerRow` moves to its own file, because it is the component that
renders the **licence**, and a second copy is where CONVENTIONS' commercial-use rule quietly stops
being true in one of the two studios.

`GenerateArtBar` is deliberately **not** a prop on `GenerateBar`, and the reason is worth being
explicit about after a lane spent arguing the other way: that bar reads a lyric document Cover Art
has none of, and every rule it enforces -- `blockers`, `canBatch`, `effectiveCount`, `queueingLabel`,
`notesFor` -- is already a shared pure function with its own test. The duplication is confined to
JSX, the one layer this repo cannot test in `node` anyway, and unifying would put three permanently
dead props in the component whose button decides what reaches Rust.

Cover Art has more empty states than the Audio Studio, because **the app ships no image profile**: a
first visit can have nothing chosen *and* nothing to choose, and those two want different sentences
-- only one of them sends the user to Setup. So `imageStudioState` names five
(`loading`/`no-profiles`/`none-chosen`/`missing`/`ready`) and `imageStudioNote` holds their wording,
as values rather than assembled inside JSX. That is the only thing in the lane a unit test can
reach, which is exactly why the sentences were pulled out of the markup.

**The click-through is eleven steps, and it reads the files rather than only the screen** -- a tile
on screen proves the webview resolved a URL, not that the record on disk is right. Steps 5 and 6 are
the point of the whole lane: the tile appearing with no reload proves the `Saved::Art` arm of
`ingest_if_pending` emits `art://saved`, and the PNG landing in `art/` rather than `tracks/` proves
`ModelKind::Image => art_dir`. Both are lines no test in `src-tauri` reaches, because they need an
`AppHandle` no test in that crate builds -- named as untested when T-506b landed, and deferred to
here on purpose. The remaining nine cover cross-panel independence (what the factory bought), the
seed on screen being the seed that ran, the sidecar's resolved slots, a batch of two, a project
switch (the reason T-506c-c refused to cache URLs by id), a broken image keeping its facts, and the
Library still working after `provenanceView` changed signature underneath it.

### 2026-09-03 (last) -- T-506d landed: Cover Art is on screen, awaiting the click-through

The view, the two extractions, `GenerateArtBar`, the five-state selector and the gallery, all
matching the brief. `ParamPanel` now takes its store as a prop, so T-506c-b's factory finally has
two live instances; `ProfilePickerRow` moved out of `AudioStudio.tsx` verbatim with a `group` prop
so the two pickers' radios are their own sets.

**The gate was not run** -- one unused-variable error stood. Two review fixes, and both are the same
shape: a value outliving the thing that produced it.

`selectedImageProfile` resolved a configured id against **every** loaded profile, so a *music* id
sitting in `default_image_profile_id` read as `ready`. That is reachable -- the two default fields
are deliberately independent (T-506c-a) and `config.json` is a file a user can open -- and it would
have shown a music param panel inside Cover Art, with nothing to stop it until `generate_image`'s
kind guard refused at submit. It now resolves against the image list, which is what its name always
claimed, and a test pins the case.

`ArtTile`'s `broken` flag never cleared. A tile whose image failed to load once stayed "Image file
not found." for the life of the mount, including after the file came back -- which the
click-through's own step 10 would have walked straight into. Reset on `row` identity: `artRows`
builds fresh row objects on every load, so it changes on exactly a gallery reload and never on an
unrelated re-render.

**Nine mutations, nine killed**, all on the one testable piece: the loading check moved after the
emptiness check (a still-loading app accusing the user of having no models), the state counting
music profiles, an unresolvable id reading as `none-chosen` and nothing-chosen reading as `missing`,
`selectedImageProfile` widened back to every profile, the `missing` sentence dropping the id it
exists to name, `none-chosen` borrowing the `no-profiles` sentence (and with it a pointless trip to
Setup), and `loading`/`ready` speaking when they should say nothing. Everything else in the lane is
JSX, which is why the sentences were pulled into `imageStudioNote` and why the eleven-step
click-through is the real acceptance.

frontend 497 -> **508**; gate green. **Next: the producer click-through**, which is what proves the
two lines in `ingest_if_pending` no `src-tauri` test can reach.

### 2026-09-03 (end of session) -- T-506d click-through passed; cover art works end to end

All eleven steps. The two that the whole phase was pointed at both hold: a finished cover appears in
the gallery **with no reload**, which is the only proof that `ingest_if_pending`'s `Saved::Art` arm
emits `art://saved`; and the PNG lands in `art/` rather than `tracks/`, which is the only proof of
`ModelKind::Image => art_dir`. Both lines were named as untested when T-506b landed rather than
papered over, and both are now discharged -- they need an `AppHandle` no test in `src-tauri` builds,
so this was always going to be a click-through or nothing.

The rest held too: the sidecar's resolved slots and its 768x768 match what actually executed (the
size slots being inert is still the expected behaviour, not a defect this lane owns); the two param
panels stayed independent across a view switch, which is what T-506c-b's factory bought; a batch of
two gave two seeds, two files, two tiles and two sidecars; a project switch changed the gallery,
which is the reason T-506c-c refused to cache URLs by id; a renamed file read as missing and kept
its facts; and the Library was undisturbed by `provenanceView`'s new signature.

**Cover art now works end to end** -- pick an image model, set a prompt, generate, and the result
files itself with a provenance record. Scoping this by running the pipeline outside the app first
(MCP-SURFACE 35) is what made the lane order right: the Rust generation half needed no change at
all, and every line of work was after the output.

**T-506e is the only lane left** in the task: attach artwork to a track (`Track.cover: Option<ArtId>`)
or an album, and `delete_art` following T-408's shape. After that the phase is packaging and
public-repo readiness -- T-507 through T-510.

### 2026-09-03 (last) -- T-506e split; e-a briefed, and the delete rule comes out the other way

Split for the reason T-506c was: the stores need a schema change and three commands that do not
exist, and mixing Rust into a store lane is what the T-504 -> T-505a cadence exists to avoid. e-a is
the backend -- `Track.cover`, `AlbumList.cover`, `set_track_cover`, `set_album_cover`, `delete_art`,
three commands, no frontend.

**The interesting part is that the phase map had the delete rule wrong, and reading the code says
so.** T-506e's entry has said since it was written that `delete_art` would carry "a
`tracks_referencing`-style check" -- the lyric-document shape, where a reference *refuses* the
delete. The repo actually holds two precedents and they point opposite ways. `lyrics::delete_doc`
refuses because a track's `LyricRef` is part of the recipe: deleting the document leaves a sidecar
naming lyrics nobody can show, and the refusal protects a record that has to stay reproducible.
`tracks::delete_track` does the reverse -- it *clears* the id from every album that holds it --
because an album is the user's current arrangement, not a record of how anything was made.

A cover is the second kind. It sits beside `title` as an editable pointer, and nothing about
reproducing a track depends on it; no record becomes unreadable when it goes. Refusing would mean
detaching a bad cover from every track and album before it could be deleted -- friction bought with
no protection at all. So `delete_art` clears, and the phase-5 entry is corrected rather than left to
mislead the next brief. The T-408 shape still governs the file half: to OS trash, trasher injected
so `cargo test` never fills a Recycle Bin, files first and record last, missing files tolerated.

Two things the brief states rather than hides. `cover` goes on `Track`, **never inside
`Provenance`** -- and a test asserts the whole provenance is untouched by setting one, because if
that can fail the field is in the wrong struct. And clearing covers on delete is **N separate atomic
writes, not one transaction**: a crash part-way leaves some tracks with no cover and some naming a
deleted artwork. Both are states the view has to render anyway, which is why it is tolerable -- and
it is named as a requirement on e-b, so a dangling cover is designed for rather than discovered.

### 2026-09-03 (last) -- T-506e-a landed: a cover is a pointer, and deleting an artwork clears it

`Track.cover`, `AlbumList.cover`, `set_track_cover`, `set_album_cover`, `delete_art` and three
commands, all matching the brief. ARCHITECTURE §8 gained the rule the lane exists to state: a cover
is a pointer, not provenance -- it sits beside `title` in the sidecar (and in `project.json` for an
album, which has no file of its own), never inside `Provenance`, which records what *made* an asset
and is never rewritten.

**The gate was not run.** The crate did not compile -- `&proj.slug` and `&mut proj` in one call --
and behind that sat four failing tests with a single cause: `add_artwork_to_project` minted an id
and wrote a sidecar but **never pushed the id onto `Project::art`**, which is the only thing the
cover setters check. So every test that set a cover hit `NotFound`, and the album test asserted the
wrong error kind while looking like it passed for the right reason. Replaced with a four-line
`register_art_id` that does what the code under test actually reads; the setters never load the
artwork, so a fixture that built one was proving nothing. A third hand-built `Artwork` literal in
`art.rs` went back onto its own `sample_artwork` at the same time.

**Fifteen mutations, fifteen killed -- two of them only after the count was made honest.**
`#[serde(default)]` on an `Option<ArtId>` is an **equivalent mutant**: serde already defaults a
missing `Option`, so both legacy-load tests pass with the attribute removed. That is the same fact
T-506c-a logged about `default_image_profile_id`, and the answer is the same -- the attribute stays,
because every other optional field in these structs carries it and dropping one for tidiness would
be the real defect. The other survivor was worth fixing: **files first, record last** had no test at
all, here or in `delete_track`, and it is not purely a crash-ordering rule -- trashing genuinely
fails on a locked file or a volume with no Recycle Bin. A trasher that succeeds on the image and
fails on the sidecar now pins the reachable half: the project still lists the artwork, so the delete
can be retried. Reordering the function kills it.

The thirteen that died first: both ownership checks dropped, the cover argument ignored, the
provenance rewritten while the sidecar was open, an unknown album silently treated as the first one,
track covers not cleared, **every** track cover cleared rather than the matching one, album covers
not cleared, the unknown-id check skipped, `next_art_seq` rewound, a hard delete in place of the
trash, and trashing a file that is not there.

create-core 191 -> **193**, library 126 -> **139**, src-tauri 133 -> **134**; gate green. Next:
**T-506e-b**, the frontend -- attach/detach on a track and an album, delete from the gallery, and a
dangling cover rendered as missing.

### 2026-09-03 (last) -- T-506e-b briefed: the cover stores, and what `missing` has to mean

The frontend half split in two along the seam this phase has used twice: e-b is every rule as a pure
function or a tested store action, e-c is the views and the click-through. Three bridge calls,
`cover` on the two wire types, and `state/covers.ts` -- its own module because these selectors
describe a cover **on something else**, and both the Library and the album panel want them without
pulling in the gallery store.

The decision worth writing down is that `coverView` has **three** states, not two. `missing` is not
`none`: `none` says the row never had a cover, which is a different claim and a false one when the
row names an artwork that has gone. It is not a hypothetical state either -- T-506e-a clears covers
in N separate atomic writes rather than one transaction, so a crash part-way leaves exactly this,
and e-a's brief named rendering it as a requirement on the frontend rather than leaving it to be
discovered. It follows the T-403 rule: render it as missing, never as an error, and never by
dropping the row.

Two smaller ones. `deleteArtPrompt` states the rule unconditionally -- the image and its record go
to the Recycle Bin, and anything using it as a cover loses it -- and appends usage counts only when
there are any, so a view that has not loaded the tracks says less rather than something untrue; a
prompt that reported "0 tracks" from an unloaded store would be a lie in a confirm dialog, which is
the worst place for one. And `art.remove` reloads **three** stores, because `delete_art` rewrites
track sidecars and album lists the frontend is already holding: skipping either leaves a cover on
screen that no longer exists on disk, which is ARCHITECTURE §8's two-files-disagreeing hazard
arriving one layer up.

### 2026-09-03 (last) -- T-506e-b landed: the cover stores, and two tests that passed for the wrong reason

Three bridge calls, `cover` on the two wire types, `state/covers.ts` with its four selectors, and
the four store actions -- all matching the brief. **The gate was not run**: two `tsc` errors, a
missing `afterEach` import and an unused `beforeEach`.

The two review fixes are both the same species, and it is the species this project keeps meeting: a
test that passes without proving what its name claims.

The new delete-flow suite reset only `mockDeleteArt`. But `remove` calls the gallery's own `load`,
so `listArt` still carried whatever resolved value the **previous** describe's last test happened to
leave armed on it -- meaning the suite's `expect(error).toBeNull()` was decided by test order rather
than by the code. It passed, and it would have kept passing until someone reordered the file. It now
resets and arms every mock it touches, and asserts the gallery reload it was named for, which the
original never checked at all.

The other came out of the mutation run: `albumRows` dropping the cover **survived**, because the
fixture gave every album `cover: null` -- so `cover: album.cover` and `cover: null` produce
identical rows and the assertion proved nothing about the join. One album now carries a real id.
This is the third time in this phase a default has quietly emptied the case a test claimed to check
(T-506c-c's `??` swallowing an explicit `null`, T-506d's `broken` flag), and the pattern is worth
naming: **when a fixture's default equals the value under test, the test is decoration.**

**Seventeen mutations, seventeen killed** after that fix -- including the three that matter most
here: `coverView` reading a dangling cover as `none` (which would claim the row never had one),
`remove` skipping any one of its three reloads, and a failed `remove` clearing the confirm so the
tile could not retry.

Also restored the house confirm wording -- curly quotes around the name, as `ProjectDelete` has had
since T-401.

frontend 508 -> **525**; gate green. Next: **T-506e-c**, the views and the click-through -- the last
lane in T-506.

### 2026-09-03 (last) -- T-506e-c briefed: the cover views, and a lane with no tests on purpose

The last lane in T-506. `CoverPicker`, the control on a track card and an album row, Delete with its
confirm on a gallery tile, and the CSS.

`CoverPicker` reads **no store**, which is the one structural decision here: its two callers write
through different ones -- `trackActions.setCover` and `albums.setCover` -- so a store read inside
would make the component pick a side, and the second caller would end up with a near-copy. Values
in, one callback out, the shape `ProfilePickerRow` already uses.

**The lane ships no tests, and the brief says so as a claim rather than an omission.** Everything
decidable already exists as a tested function in `covers.ts`; what is left is markup, and this repo
runs vitest in `node` with no DOM. So "no new exported function anywhere -- if a decision is needed,
stop and ask" is an acceptance criterion, not advice. That is the same bargain T-506d struck when
the five studio states were pulled out of JSX into `imageStudioState`, and it is why the
click-through is the acceptance for a lane like this rather than a formality after it.

One thing worth having caught in the brief rather than in review: Cover Art loads the **library and
albums** stores on mount. It looks like over-fetching for a gallery, and it is not -- `deleteArtPrompt`
can only name what a delete will detach if those two are loaded. e-b made the prompt degrade safely
when they are empty; this lane makes sure they are not, because a confirm dialog that understates
what it is about to do is the worst place in the app for a half-truth.

Twelve click-through steps, reading the files rather than the screen. The ones carrying real weight:
the track sidecar's `provenance` **unchanged** by attaching a cover (the rule the field's placement
exists to keep); three views settling with no manual reload after a delete (what `remove`'s three
store reloads buy -- one skipped leaves a cover on screen that is gone from disk); the trashed files
still in the Recycle Bin rather than erased; and a hand-made dangling cover rendering as missing and
**repairable from the UI**, which is the state e-a's non-atomic cover clearing can genuinely leave
and which e-a's brief named as a frontend requirement so it would be designed for rather than
discovered.

### 2026-09-03 (last) -- T-506e-c landed: the cover views, pending the click-through

`CoverPicker`, the control on a track card and an album row, Delete with its confirm on a tile, and
the CSS. Both acceptance criteria hold: the only new export is the component, and it reads no store.

**The gate was not run.** `art` was subscribed in `Library()` but used inside `TrackCard`, which is
a separate component -- it did not compile. Moved the subscription to where it is read.

The other three fixes were all the same thing: **deleted comments**. The run stripped the T-409
sanitise-before-the-dialog note, the entire `TrackDetails` doc comment, and the `broken`-reset
comment written during T-506d's own review -- none of them near anything the brief asked it to
change. All restored. Worth logging because it is a new failure mode from this executor: earlier
lanes deleted *code* it had not been told about (`pickable` in T-506c-b); this one deleted the
reasons. The reasons are the more expensive loss, because nothing fails when they go.

Two small alignments: `AlbumPanel` imported its sibling as `'../components/CoverPicker'` where every
other sibling import in that directory is `'./X'`, and the cover name span had no class where every
other element in the file has one.

**No mutation run, and that is the lane's own claim rather than a shortcut**: it adds no decidable
logic -- everything it renders comes from `covers.ts`, which was mutation-tested in e-b. That is why
the twelve-step click-through is the acceptance for this one. frontend stays 525.

### 2026-09-03 (end of session) -- T-506e-c click-through passed; T-506 is complete

All twelve steps. A cover attaches to a track and to an album and clears again, with the track
sidecar's `provenance` **untouched** -- the rule the field's placement beside `title` exists to
keep, and the reason `cover` was never allowed inside the recipe. Deleting an artwork that is in use
names both counts in its confirm, then settles **three views with no manual reload**, which is what
`art.remove`'s three store reloads buy: skip one and a cover stays on screen that is gone from disk.
The trashed files are in the Recycle Bin rather than erased (T-408's rule, now covering the fifth
kind of created content). A hand-edited dangling cover renders as missing and is **repairable from
the picker** -- the state T-506e-a's non-atomic cover clearing can genuinely leave, named in that
brief as a frontend requirement so it would be designed for rather than discovered, and it was.
Ids are still not reused after a delete, and covers follow their project across a switch.

**T-506 is complete.** Cover art generates, files itself with a provenance sidecar, appears without
a reload, attaches to tracks and albums, and deletes cleanly. Five lanes, two click-throughs, and
the thing that made the lane order right was scoping it by running the adopted Klein profile end to
end **outside the app** first (MCP-SURFACE §35): that found the Rust generation half needed no
change at all and that every line of work was after the output -- so storage and ingest came first,
not generation.

Phase 5 now has four tasks left, none of them feature work: **T-507** first-run and empty-state
polish (carrying the "Undeclared" `comfy.models` item from T-505d-d's click-through), **T-508**
installers, **T-509** THIRD-PARTY-LICENSES, **T-510** public-repo readiness.

### 2026-09-03 (session close) -- a full docs drift audit before handoff

Every doc read against the repo rather than against itself. Counts re-verified from a live
`npm run gate` (create-core 193, library 139, mcp-bridge 98, llm-bridge 35, src-tauri 134, frontend
525), the three git tags confirmed to be exactly `phase0-done`/`phase1-done`/`phase2-done`, the five
view files confirmed against ARCHITECTURE's nav list, `.player`'s `position: sticky` confirmed
before writing down that T-502 closed its CSS-TODO item, and every relative link in every tracked
`.md` resolved by script (all clean; the only misses were in the gitignored aider log).

**The worst drift was in AGENTS.md, and it is worth naming the mechanism.** Item 5 -- the "which
phase, which task" line -- had grown into a wall of per-task status ending in "**Next: Phase 5** ...
**not yet opened**", written while Phase 4 was closing and never revisited. Phase 5 has since been
opened and had six tasks land in it. Nothing failed; the line simply became false one session at a
time, each session appending rather than replacing. That is the failure mode the session-start
`git log` check is **worst** at catching, because the drift is not in the diff -- it is in a file
nobody touched. So item 5 is now one sentence naming the open phase, and "this file carries no
per-task state" is a hard rule with the reason attached.

Also fixed: README's Status still said Phase 4 was in progress and its data-dir table did not list
`art/`; it gained a Model catalog bullet (a headline feature that was never in the feature list) and
a real Cover art one. ROADMAP's Phase 3 heading had no completion marker and stopped at T-315
(it ran to T-317), its Phase 4 section repeated the "Phase 5 not yet opened" claim while the Phase 5
section below it said OPEN, and Phase 5 had no landed-so-far line. ARCHITECTURE said the mixing
handoff protocol "will exist before this repo's Phase 4" -- Phase 4 came and went, so it now records
what actually happened. CSS-TODO's player item was closed by T-502 and is struck through with the
task rather than deleted. phase-5.md's build-order note was written as a future decision and is now
the record of what ran, and its milestone check says which half is discharged and which waits on
T-508.

CLAUDE.md was a single pointer line. It still delegates to AGENTS.md and still holds **no** state --
that restraint is now written into it with the reason -- but it names the session-start ritual and
summarises the three rules where a mistake is unrecoverable (ship no models, no silent rewrite of a
user's words, no hard deletes), so a session that reads only its auto-loaded file cannot make one.

The repo is ready to hand off: gate green, tree clean, PROJECT.md's Snapshot carries a **Next up**
bullet, and T-507 is the first unfinished task.

### 2026-09-05 -- T-507a + T-507a-2: the empty/degraded-states sweep, and Lyrics learns reachability

T-507's frontend half. The audit read all five views against ARCHITECTURE section 10 ("status
pills, not modal walls") with **CoverArt** (T-506) as the reference: Setup, CoverArt and Library
already handle their cold-start/offline states; the two gaps were **LyricsStudio** (silent about a
missing lyrics model -- the LLM is optional and skippable -- until a generation fails) and
**AudioStudio** (a bare empty profile list before models load). **T-507a** added a pure
`lyricsModelConfigured` cue + Open Setup + disabled Generate/Optimize, and Audio's "Checking for
installed models..." line. Frontend 525 -> 530.

**The click-through corrected the brief's scope**, the useful way. With `config.llm.model` holding a
stale `qwen3.5-27b` from a past session, the cue stayed hidden -- the app correctly saw *a* model
configured -- and Generate failed into a raw banner. The proactive cue as written only ever fires
for a truly first-run user who never touched the LLM step; the state a real user hits is *configured
but unreachable*. Owner chose (decisions log, 2026-09-05) to **reuse Setup's existing probe** rather
than add a mount-time probe (the section 10 rule the owner set in Setup): **T-507a-2** added a
`lyricsModelState(model, status)` machine over the llm store's status -- `none` / `unreachable` /
`not-offered` / `ready` / `unknown` -- disabling the two actions and showing a cue for the three
blocking states, while `unknown` (no probe read yet) preserves prior behaviour. The reason it works
without a probe here: **the app opens on Setup, whose LlmStep probes on mount**, so the status is
populated from launch. Frontend 530 -> 541.

Review caught Aider making **two out-of-scope edits** -- flipping unrelated `prefillFrom` tests to
`async` with no `await`, which would have tripped oxlint's `require-await` -- reverted before commit.
**Both click-throughs passed 2026-09-05, all four steps** each: the stale-model case reads "Model
unavailable" naming the model, a downed endpoint reads "Model unreachable", a model the endpoint
offers clears the cue, and clearing the model entirely still shows "No lyrics model". **T-507's
frontend half is complete; T-507b (the adopted-profile `comfy.models` fix) is the one piece left.**

### 2026-09-05 (later) -- T-507b landed, then the catalog pivot, then T-511 (five curated image models)

**T-507b** landed and click-through-passed: `emit_profile` now enriches an adopted profile's
`comfy.models` by enumerating the model folders (`list_model_folders`), building the same
`ModelInventory` readiness trusts, and matching the graph's COMBO slot values against it -- the
inventory match is the filter, so an adopted gallery row reads **Ready** instead of "Cannot check".
`build_profile` stays pure; the resolution lives at the live seam. create-core 193 -> 198.

**Then the owner reframed the whole catalog** (two decisions-log entries, 2026-09-05). The catalog
existed to *install* models from the app, and the image side could not: the app shipped two audio
profiles and **zero image profiles**, so every image row was adopt-only. Re-verifying comfy-mcp live
(MCP-SURFACE §36) confirmed it still cannot auto-install an arbitrary gallery model -- `download_model`
is URL-only, a template exposes no model URL, and ComfyUI-Manager's model DB is not exposed. So the
catalog is refocused on **what the app can install**: a curated list surfaced through the existing
Setup Models step, and the full gallery browse is removed (**T-512**). A first model shortlist was
made, then **corrected on a licence check** -- Flux.2 Klein 9B and Flux.1 Dev are **non-commercial**,
unusable for cover art on released music -- so the set is **commercial-safe only**.

**T-511 complete: five curated image profiles**, authored as verified content (not Aider), each
`fetch_template` -> `list_workflow_slots` for the exact files/addresses and each download URL
confirmed live on Hugging Face to be **ungated** (a no-auth `download_model` needs that) and
commercial-safe: **Flux.1 Schnell FP8** (Apache-2.0), **Chroma1 HD** (Apache-2.0, 3 files -- its Flux
VAE taken from an ungated Comfy-Org repo because the official BFL schnell repo is gated), **SDXL 1.0**
(Open RAIL++-M), **SD 3.5 Large FP8** (Stability Community), **Qwen-Image** (Apache-2.0, 4 files
including an 8-step Lightning LoRA; the high-VRAM option at `vram_gb_min` 24, and the one that could
**not** be run-verified on the 16 GB dev card -- its promoted-subgraph fast/quality switch is a
BOOLEAN the profile schema has no input type for, so it ships at the template default). All five
install one-click through the Models step (`curatedFirst(view.profiles)`, no kind filter) and are
pinned in `library`'s shipped-profiles guard test, which now also checks non-empty display_name and
licence since curated profiles are hand-authored. **T-512** (strip the gallery) is the remaining
catalog work; import-your-own-workflow (T-313) stays.

### 2026-09-05 (later) -- T-512 stripped the gallery; T-513 broadened the audio catalog

**T-512** removed the gallery browse now that the catalog is curated-only: deleted `ModelCatalog`,
`state/catalog` (+test), `bridge/catalog`, and `src-tauri/catalog.rs` with its three commands, and
cut only the adopt pieces from the shared import code (`adopt`/`adopting`/`catalogAdoptBegin`) so
import-your-own-workflow (T-313) is untouched -- its store tests were rewritten to cover the
file-import `begin`/`reset` path directly. The Setup Models step gained an Audio | Image split and is
now the whole catalog. Frontend 541 -> 522, app 134 -> 129 (deleted code's tests). ARCHITECTURE
§10/§10a/§5b and README were rewritten to match. **Awaiting click-through.**

**T-513** closed the audio/image asymmetry the owner spotted: T-511 shipped five image models but the
audio side was still just ACE Turbo (a test pick) + MiniMax. Added **ACE-Step 1.5 XL Base** and
**SFT** -- the non-distilled variants most people run -- each a clone of the Turbo profile (swapped
unet, 50-step default; shared encoders/VAE/lyrics-contract/LoRA). **Stable Audio was tried and
deferred** (decisions log): SA 3.0's gallery template is an LLM-prompt-expansion subgraph comfy-cli
flags as slot-unreliable, and SA Open 1.0's `t5-base` encoder has no clean ungated ComfyUI source --
both backlog rather than a shipped guess. The curated set is now **nine models** -- audio:
Turbo/Base/SFT/MiniMax, image: Schnell/Chroma/SDXL/SD3.5/Qwen -- all commercial-safe, all pinned in
the guard test. The method throughout: `fetch_template` ->
`list_workflow_slots` for exact files/addresses, every download URL verified live on HF as ungated.

### 2026-09-06 -- the Setup/studio design pass (T-514), and the remote-ComfyUI gap named (T-515)

An owner design session after a stretch of using the app, **run without Aider** (owner's call --
the work was many small coordinated edits across five views, one component family and one
stylesheet, which is the shape a brief-per-run loop handles worst).

**The complaint, and what was actually wrong.** Three separate-looking asks -- make Setup's audio
list look like Audio's, Setup's image list look like Cover Art's, and move Library's project list to
Setup in the same style -- turned out to be one thing on inspection: **Audio and Cover Art already
shared a list**. Both render `ProfilePickerRow` into `.profile-list`, and `.profile-list` and
`.project-list` were byte-identical rules. The odd one out was the Setup Models step, which had its
own `.model-row` layout and no scrolling window. So "three styles" was one style plus one fork, and
the fix was to delete the fork rather than to build two more.

**What landed (T-514).** One `.picker-list` / `.picker-row` family for every list a model or project
is chosen from; `ProfilePickerRow` takes `children`, so Setup hangs its file list, download progress
and **Install** *inside* the shared row instead of forking the style again. Setup's rows became
**selectable** (they write `default_profile_id` / `default_image_profile_id`), Setup gained a
**Projects step** (list, create, delete, moved off Library), and `ImportWorkflow` moved off Audio --
model management now lives in one place. The four Setup cards got the `margin-bottom` they never
had; they had been running together as one long card.

The studios got **`QuickSwap`**, a name-only dropdown over installed models, on Audio, Cover Art,
Lyrics and Library. Two rules in it earn their tests and are the kind of thing this project has been
bitten by before: an **unreadable inventory offers everything, not nothing** (`unknown` is "could
not check", not "not installed" -- filtering on `ready` would hand a user with a perfectly good
install an empty menu), and the **chosen id is always among the options** even when uninstalled,
because a `<select>` whose `value` matches no option renders the *first* one -- the control would
silently claim a model the user never picked, the same fault as substituting a default.

**Owner decision recorded** (decisions log): the studio dropdown writes the **same config field** as
the Setup radio. One value, so the two screens cannot disagree, and a swap survives a restart.

**Library** also got the two things the owner asked for: **Play leads the row** (a round icon button
before the title; Send-to and the duration are pushed right by `margin-left: auto`), and the loaded
track carries a **`nowPlayingLabel`** tag. That one is worth keeping the reason for: the transport
shows a title and nothing else, so eight takes named "Midnight" give eight identical play bars, and
the tag is the only way to tell which row the transport is holding. It marks the **loaded** track,
not only the playing one -- a paused or finished row still has to be findable -- and the row
subscribes to the *label*, not to player state, so a `timeupdate` several times a second does not
re-render the whole list.

Frontend **522 -> 543** tests. Dead rules removed with their markup (`.profile-*`, `.project-list`
/`-row`, `.model-row*`); `.profile-picker-setup` renamed `.setup-link`, the last survivor of a
family that no longer exists. Two stale strings fixed on the way, both left over from the 2026-09-05
catalog pivot: "Bring one in from the model catalog in Setup" (adopt language for a feature T-512
deleted) and two "Pick one below" cues that now point at a menu above them.

**T-515 named, not built.** The owner has raised remote/API ComfyUI several times across this
project and it had never been written down. Checking rather than assuming found the useful half:
**the config schema already carries it and nothing reads it** -- `ComfyMode::{Local, Cloud}`,
`ComfyConfig.url` and a whitelisted `comfy_cloud_api_key` keychain secret all exist, and
`ComfyMode`'s only readers are its own unit tests; `src-tauri/src/comfy.rs` builds a `LocalComfy`
unconditionally. So the task is wiring plus a live surface check. It is scheduled as its own session
because two of the check's answers reshape screens rather than adding a field: comfy-mcp's
`comfy_target` (self-hosted remote) is a **different product** from Comfy Cloud and `ComfyMode::Cloud`
names only the second, and **MCP-SURFACE 36 already records that `download_model` refuses outright
...
### 2026-09-06 (later) -- T-516: tags and cover art on an exported track

Built the same day it was scoped, after the owner answered the two questions the research left
open: **a full tag set** rather than artist alone, and **export only** for now.

**Only the artist was actually missing.** Title is on the track, album and track number come from
the album list it sits in, the year from its own `created_at`, and the artwork from `Track.cover`.
So the new configuration is one field -- `config.export.artist`, with a **Release details** card on
Setup -- and everything else is a read of what the app already had. That is why this landed in a
session rather than a phase.

**Three rules, each with a test that names it.** The tags go on **the copy only**: the file under
`tracks/` stays byte-for-byte as generated, because it is the artifact its provenance sidecar claims
to describe, and tagging it would silently change the thing the recipe reproduces. **Tagging never
fails an export**: a deleted cover, an unreadable image, a format `lofty` will not write -- each is a
warning beside a file the user still gets, surfaced on the row as a caution rather than an error.
And the album is the **first list containing the track**, in the project's order: a track can sit in
several, an export names a track rather than a release, and stable beats arbitrary.

**Verified on real data, not only fixtures.** Exporting a real MiniMax track from the developer's
own library gave `AlbumTitle="thisIsATestAlbum"`, `TrackNumber=1`, `Year=2026`, the model/licence
comment, a **1,975,060-byte PNG embedded as `CoverFront`**, ComfyUI's own
`EncoderSoftware="Lavf62.12.102"` **preserved** rather than replaced, and the library original
**unchanged at 12,902,559 bytes**. Two tiny committed fixtures keep that path in CI without shipping
megabytes: `testdata/audio/silence.flac` (42 bytes -- `fLaC` plus a STREAMINFO block and no frames)
and `testdata/images/tiny-cover.png` (69 bytes, 1x1), both generated rather than captured.

**`config.json` gained an `export` section without a schema bump** -- `#[serde(default)]`, so an
existing file loads and gains an empty one, which is what a user who never set an artist should get.
The cross-language wire fixture failed the moment the shape changed and named both files to update,
which is exactly the job it was built for (T-006).

Also in this pass: the **Albums panel moved to the top of the Library**, under the project menu.
Both answer "what am I working on", and at the foot of a long library the panel was off the bottom
of the screen.
...
### 2026-09-06 (close) -- the className check is in the gate; all click-throughs cleared; handoff

**The audit became a test.** `app/src/theme.test.ts` fails the gate when a className used in TSX has
no rule in `theme.css` -- the rule CONVENTIONS has always stated and nothing has ever checked. It is
a vitest test rather than a new gate step, so it runs in the suite that already runs and reports
like any other failure. Three details earned their code:

- **It reads the interpolations.** `` `picker-row ${selected ? 'picker-row-selected' : ''}` `` is
  where half this app's state classes live, so a scanner that stripped `${...}` wholesale would check
  the least-exercised half of the stylesheet and miss the rest.
- **A computed suffix is checked as a prefix.** `` `job-item-${row.status}` `` passes only if some
  rule actually answers to `job-item-`.
- **The exemption list is kept honest by its own test.** `UNSTYLED_BY_DESIGN` holds the two classes
  that really are styled by a parent's flex layout (`.import-role`, `.track-details`), each with its
  reason, and a third test fails if an entry stops being used or starts being styled -- otherwise a
  stale exemption becomes the hiding place for the next genuinely unstyled class.

**Verified by breaking it**, not by watching it pass: deleting the `.art-gallery` rule -- the real
bug from earlier the same day -- fails the test naming the class and the file. Frontend 549 -> 552.

**Every outstanding producer click-through passed** (owner, 2026-09-06): **T-516** (exported files
carry all the proper tags), **T-514** with its three follow-up rounds, and **T-512**, which had been
waiting since 2026-09-05. **T-505c** is discharged rather than run: its own click-through targeted
the gallery UI that T-512 later deleted, while the install path it added survives and is
owner-verified end to end. **Nothing in this repo is now waiting on the owner.**

**Handoff.** Phase 5 has **no feature work left**. What remains is the release arc -- **T-508**
installers (the phase milestone, and the only thing between this repo and a person who is not the
developer), **T-509** THIRD-PARTY-LICENSES (now including `lofty`), **T-510** public-repo readiness --
plus **T-515**, remote ComfyUI, a v1 blocker that wants its own session and blocks the version rather
than the milestone. The Snapshot's "Next up" carries the order and the reasoning; `tasks/phase-5.md`
carries the scope. The next session should still start the way every session starts: PROJECT.md
Snapshot, then this entry, then `git log` against both.

### 2026-09-06 (later) -- T-508 landed: the release workflow

**The milestone's build half is in place.** `.github/workflows/release.yml` builds the four
installers -- Windows NSIS `.exe`, macOS `.dmg` (Apple Silicon + Intel), Linux AppImage -- via
`tauri-action`, on a `v*` tag push or manual run, and attaches them to a **draft** GitHub release.
Two owner decisions shaped it, both taken at the top of the session: **unsigned first** (no code
signing, no MSIX -- SmartScreen and Gatekeeper will warn; signing is a later task gated on the owner
providing a Windows cert and an Apple Developer ID), and **all three platforms at once** (the CI
matrix already runs all three OSes, and `tauri-action` makes the extras cheap).

**The one config change is ad-hoc macOS signing.** `tauri.conf.json` gained
`bundle.macOS.signingIdentity: "-"`, the documented way to stop macOS reporting an unsigned Apple
Silicon build downloaded from GitHub as "damaged" (it still needs the right-click → Open whitelist,
but the app is not reported broken). Verified against the live Tauri docs, not recalled -- the
`signingIdentity` field and the `-` pseudo-identity are both in the macOS signing guide.

**The bundle config was already complete.** `targets: "all"`, the icon set (including the Windows
Store logos), `licenseFile`, and the `profiles/*.json` resource were all present from earlier phases;
nothing there needed to change. The workflow reuses the `ci.yml` conventions (node 22, the same
Linux package set, rust-cache keyed on platform) and adds `patchelf`, which the AppImage bundler
needs and `ci.yml` never installed because it never bundles.

**This is a CI-only change and cannot be exercised by `npm run gate`** -- the gate deliberately does
not run `tauri build`. The real check is the milestone: the owner's one `v*` tag push producing four
installers on a draft release. The workflow is written to fail loudly (matrix `fail-fast: false`) so
one platform's bundling problem does not hide the others. `npm run gate` is green; the `tauri.conf.json`
change is read by `tauri-build` during compilation, so the gate did exercise it.

**What remains in the release arc:** T-509 (THIRD-PARTY-LICENSES) and T-510 (public-repo readiness),
then T-515 (remote ComfyUI) in its own session.
...
same images load. Audio was unaffected.

**The cause is one missing CSP directive, and it could not have been caught before packaging.**
`app.security.csp` was written for T-402's audio playback and named `media-src asset:
http://asset.localhost`. When T-506 added cover art four months later nobody touched it, so `img-src`
was never declared and fell back to `default-src 'self'` -- which blocks every
`http://asset.localhost` image the asset protocol serves. **Tauri injects the CSP only into responses
its own protocol serves**, and a dev run loads the frontend from Vite's dev server, so **`npm run
dev` applies no CSP at all**. The gate, every unit test and all fourteen producer click-throughs ran
against a page with no CSP on it. T-508's own log entry had already written down the shape of this
blind spot -- "this is a CSP-only change and cannot be exercised by `npm run gate`" -- one task
before it bit.

**Measured, not recalled.** The shipped `app.exe` was read with `strings` to confirm it carries the
old CSP verbatim, and the directive's behaviour was then observed directly: two pages served over
HTTP with the old and new CSP, each with a `securitypolicyviolation` listener and an
`http://asset.localhost` image. The old page reports **`img-src blocked
http://asset.localhost/...`** and fires `onerror` -- which is exactly the `onError` that renders
"Image file not found." in `CoverArt.tsx` and `CoverPicker.tsx`. The new page reports **no violation
at all**. The probe also showed, incidentally, that this CSP blocks *inline* scripts (`script-src`
falls back to `'self'`); the app is unaffected because Vite emits an external module script.

**The fix is one directive and the only regression guard that can exist.**
`img-src 'self' asset: http://asset.localhost` (`'self'` is needed for the favicon, which had been
riding on `default-src`), plus `test_csp_covers_every_asset_protocol_directive` in `src-tauri`, which
asserts that **both** `img-src` and `media-src` name **both** spellings -- `asset:` for macOS and
Linux, `http://asset.localhost` for Windows. Mutation-checked: deleting the directive fails the test
with `CSP has no img-src`. The rule is now written into ARCHITECTURE section 11, where the frontend
rules live, because the next element type to load through the asset protocol will hit the same trap.
A full sweep of the rest of the CSP found nothing else missing: there are no `url()` in `theme.css`,
no `fetch`/WebSocket/`createObjectURL`/`data:` URIs anywhere in `app/src`, no iframes, and the only
