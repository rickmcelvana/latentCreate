...
- **Next up (2026-09-05):** the catalog rework is essentially done; **T-508** installers is the next real task. The **2026-09-05 catalog pivot** (decisions log) made the catalog a **curated installable list** surfaced through the Setup Models step, with the gallery browse removed. Landed since: **T-511** -- five commercial-safe **image** profiles (**Flux.1 Schnell FP8** Apache-2.0, **Chroma1 HD** Apache-2.0, **SDXL 1.0** Open RAIL++-M, **SD 3.5 Large FP8** Stability Community, **Qwen-Image** Apache-2.0/high-VRAM), every URL/size verified live and ungated; **T-512** -- stripped the gallery (T-504 backend, T-505a/b UI, T-505d adopt deleted; import-your-own-workflow T-313 kept), Models step split Audio | Image (**awaiting click-through**); **T-513** -- two **audio** additions, **ACE-Step 1.5 XL Base + SFT** (Apache-2.0), so the audio catalog is Turbo/Base/SFT/MiniMax. **Stable Audio deferred to backlog** (decisions log: SA 3.0's template is slot-hostile, SA Open 1.0's `t5-base` has no clean ungated source). All shipped profiles pinned in `library`'s guard test; **Schnell verified end-to-end by the owner** (installed from the app, generated). Remaining: **T-508** installers (phase milestone), **T-509** THIRD-PARTY-LICENSES (must carry all nine curated model licences), **T-510** public-repo readiness. No feature work remains.
