...
- **Phase:** **0-3 complete**, tagged `phase0-done` (2026-08-23), `phase1-done` (2026-08-25), `phase2-done` (2026-08-26); Phase 3 closed 2026-08-30 (T-301 ... T-317, all five milestone lines discharged -- the pipeline generates audio live on both profiles, ingests a finished track with a provenance sidecar checked field-for-field against the executed graph, batches by seed, imports a user's own workflow, and fails cleanly on a mid-job crash). **Phase 4 (Library & Player) is COMPLETE -- closed 2026-09-02** -- [tasks/phase-4.md](tasks/phase-4.md). Every task landed and passed its own producer click-through: T-401 (projects first-class), T-402 (playback + AnalyserNode visualizer), T-403 (album lists), T-404 (Send-to link-out), T-405 (track actions: delete-to-trash / rename / export / reveal), T-407 (shared scrollbar CSS), T-408 (delete for every kind of created content), T-409 (the song title carried to the track and the exported file), T-406 (provenance inspector + "re-use these settings"). The milestone check (generate -> play with visualizer -> album list -> send-to) was met by 2026-09-01; the last three tasks were owner-decision additions. **Not git-tagged** (Phase 3 set the recent precedent of a docs-only close). **Next: Phase 5** (cover art, polish, packaging -- T-501...), **not yet opened;** its phase-start check and `tasks/phase-5.md` are the next session's first work. **T-409 (the song title, carried) is COMPLETE** -- three lanes landed 2026-09-01 (the field on `GenerationSpec` flowing to `Track.title`, a Title input in Lyrics Studio, an Audio Studio title override + export-name sanitising), click-through passed. **T-406 (provenance inspector) is COMPLETE** -- both lanes landed 2026-09-02 (lane a a Details inspector over the whole sidecar; lane b "re-use these settings" loading a past spec into the Audio Studio with the seed pinned), **click-through passed 2026-09-02, which closed Phase 4.** **T-408 (delete for every kind of created content) is COMPLETE across all four parts (a lyric version, b lyric document, c album, d project), every one click-through-passed 2026-09-01.** part **a** (delete a lyric version, refusing when a track references it) is **complete** -- **a-back** landed 2026-09-01 architect-direct (six mutations killed), **a-front** (the Lyrics Studio delete affordance) the same day, and the **producer click-through passed all five steps** (including v31 correctly refused, naming the track); one message-placement defect found in it and fixed (the refusal now renders inline at its row, not the top of a 31-item list). **T-408b is code-complete:** its **backend landed** 2026-09-01 architect-direct (`delete_doc`, a shared `tracks_referencing` helper, the `lyrics_list`/`lyrics_create`/`lyrics_delete_doc` commands, `lyrics_open` now takes an optional id; six mutations killed), and **b-front** (the Lyrics Studio document picker: switch / New / Delete-document, multi-document store) landed the same day; **its producer click-through passed all five steps 2026-09-01.** **T-408c (delete an album) is code-complete:** it lands the pattern-breaker of the group -- **an album has no file** (it is a named entry in `project.albums`), so `delete_album` trashes nothing and has no reference to refuse against; deleting a list never touches its tracks. Landed 2026-09-01 architect-direct (three mutations killed): `delete_album`, the `album_delete` command, and an inline-confirm Delete in the album panel whose copy reassures the tracks stay; **producer click-through passed all steps 2026-09-01 -- a deleted album's tracks all stayed in the library, so T-408c is complete.** **T-408d (delete a project) is complete** -- the last part, and the one that took the pattern furthest: **a project *is* its directory**, so `delete_project` trashes the whole `projects/<slug>/` tree in one move (no file-first/record-last order to protect), checks existence on the **directory not the record** (a project with a malformed `project.json` must stay deletable), and **does not touch the config selection** -- deleting the *selected* project leaves `config.default_project_slug` naming a gone slug, which `selected_project` and `effectiveProjectSlug` already resolve to the first remaining project, the fallback the decisions log flagged (2026-08-30) and the first flow to reach it from a real deletion. Landed 2026-09-01 architect-direct (three destructive-core mutations killed via file-copy backup): `delete_project`, the `projects_delete` command, a `projectctx` wiring test proving a deleted selected project lands on a sibling, and a strong-warning inline-confirm Delete on each project row. **Its producer click-through passed all five steps 2026-09-01** (a non-selected project deleted cleanly; deleting the *selected* project landed the app on another with no restart; the trashed project folders were whole in the Recycle Bin), so **T-408 closes.** Counts: create-core 175, library 109, mcp-bridge 96, llm-bridge 35, src-tauri 114, **frontend 435** (T-408d 414 -> T-409 426 -> T-406 435); `npm run gate` green, tree clean. Per-task history is in the session log below, not here.
