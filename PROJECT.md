...
- **Phase:** **0, 1 and 2 complete**, tagged `phase0-done` (2026-08-23), **`phase1-done` (2026-08-25)** and **`phase2-done` (2026-08-26)**. Phase 2 (Lyrics Studio) shipped T-201 … T-210 plus the T-211 live milestone and its fix-ups (T-212 … T-214): a user fills in a brief, watches lyrics stream from their own LLM, edits across versions, runs an advisory structure lint, optionally accepts a consent-gated optimized prompt, and approves a version for audio. **Verified live on 2026-08-26** against a real Ollama -- a whole song in 6.9-9.2 s with reasoning suppressed (against a 44 s / 85-character baseline), the optimizer preserving its five fixed lines 5/5, and the approved lyric on disk with its `prompt_optimized` consent flag. `mcp-bridge` (88 offline tests) covers the verified comfy-mcp surface; `llm-bridge` (35 + 4 live) covers OpenAI-compatible streaming plus Ollama's native API. **Phase 3 is in progress** -- the pipeline is built and **has generated audio live on both profiles** (2026-08-28, MiniMax and ACE-Step with two LoRAs stacked), and the queue panel that reports those jobs is on screen and click-through-verified (T-310a-d, 2026-08-29). **T-313 is complete (a-g) and click-through-verified 2026-08-30**: a person can import their own ComfyUI workflow, confirm what the app guessed about it, save it as a profile indistinguishable from a shipped one, and generate from it. **All five ROADMAP milestone lines are now discharged.** **T-314 is complete 2026-08-30**: all three producer runs done. The gallery arm still generates after T-313a's refactor; the project's **first recorded full-length generations** ran **185 s and 200 s of audio in 36-40 s wall clock** (~5x realtime), every sidecar's `duration_s` exactly matching its FLAC; and VRAM was measured at 1 Hz to a **peak of 15.49 GiB of 15.93 GiB**. **`vram_gb_min` stays at 8 deliberately** -- ComfyUI expands to fill free VRAM, so an unconstrained run measures the card, not the model (MCP-SURFACE 30.3). **T-316 landed 2026-08-30** (a fresh Generate re-rolls the seed unless the user pinned it), **T-317 complete 2026-08-30** (the constrained bisect: ACE-Step never fails, it offloads and slows down -- `vram_gb_min` is a comfort floor, not a "will it run" gate, MCP-SURFACE 31), and **OQ-3 closed 2026-08-30** (raw ComfyUI API fallback: no for v1). **Phase 3 is complete** -- T-301 … T-317 all landed, all five ROADMAP milestone lines discharged. **Phase 4 (Library & Player) is in progress** -- [tasks/phase-4.md](tasks/phase-4.md) written 2026-08-30 with the phase-start check done (the mixing/mastering repos are web-first, no handoff protocol, so Send-to stays the v1 link-out) and two owner decisions recorded (projects become first-class; milestone-first ordering). **T-401 is complete 2026-08-30** — T-401a (the backend seam:
`default_project_slug` in config, `projectctx::default_project` renamed `selected_project`,
...
the player bar. **T-402d landed 2026-08-30**: the click-through on a built app found the track
...
**T-403 (album lists, the second milestone line) is complete (a/b/c) and its click-through
PASSED 2026-08-31** -- `library::albums` (create/rename/add/remove/reorder over `Project.albums`),
the six `albums_*`/`album_*` commands, the `state/albums` store with pure
`albumRows`/`moveTrackId` (18 tests), and the Library album panel. The producer ran all six
click-through steps (duplicate names refused, add picker filters, reorder persists, remove
returns to the picker, a deleted track renders as "Missing track", rename follows the open album)
and **the second Phase 4 milestone line ("album list") is discharged.** T-404 … T-406 planned,
not briefed. **Planning pass 2026-09-01** added three owner decisions and two tasks: T-404 ships as the v1 link-out now (the sibling apps still have no import surface), **T-408** widens delete to every kind of created content (lyric versions, lyric documents, albums, projects -- the app has none of them today, and a lyric version a track points at is **refused** rather than deleted), and **T-409** carries a song title from Lyrics Studio through `GenerationSpec` to `Track.title` and the export filename (both title fields exist in the schema and neither is ever written). Order: T-404 -> T-405 -> T-408 -> T-409 -> T-406. **T-404 is briefed and its backend has landed** ([t-404-brief.md](tasks/t-404-brief.md)): T-404a (`src-tauri/src/sendto.rs`, the `send_to` command, `target_url`, four tests -- src-tauri 107 -> **111**) is architect-direct and in the tree; T-404b (the frontend affordance) landed 2026-09-01 -- frontend 373 -> **382** -- with three review defects fixed directly: a deleted `.track-head-actions` rule that `tsc`/`oxlint`/vitest cannot see, and two surviving mutations (a hardcoded destination and a missing success-path reset) that the run's own seven tests left green. **Its click-through PASSED 2026-09-01, all five steps** -- both destinations open their own site with the file revealed, a missing file gives the mapped sentence and opens nothing, the error moves with the row rather than multiplying -- so **the third and last Phase 4 milestone line is discharged and the phase's whole milestone check is met**. The phase itself continues: T-405, T-408, T-409 and T-406 remain. **T-405 is briefed and its backend has landed** ([t-405-brief.md](tasks/t-405-brief.md)): T-405a added `library::tracks::delete_track` / `rename_track` / `export_track` / `trash_to_os`, the `trash` (MIT) dep, `LibraryError::Trash`, and the four Tauri commands -- **library 74 -> 84**, 10 tests, 5 mutations killed. T-405b (the action store) and T-405c (the `TrackCard` controls) **landed 2026-09-01** -- frontend 382 -> **395** -- with the two failure tests fixed to arm their precondition (they passed vacuously; the store was byte-identical to the reference and correct). **Its click-through PASSED 2026-09-01, all six steps** -- delete to the OS trash (both files), no id reuse, rename persist/clear, export copy-not-move, reveal-selects, per-row error -- so **T-405 is complete**. Phase 4 continues with T-408, T-409 and T-406. Briefing it verified that **no capability change is needed** and that **ARCHITECTURE 8's URLs are correct** -- `app.latentmixer.com` is the deployed mixing address, and the 59-to-17 majority for `latentmixing.com` in that repo's docs is stale.
