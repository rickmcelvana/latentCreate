...
not briefed. **Planning pass 2026-09-01** added three owner decisions and two tasks: T-404 ships as the v1 link-out now (the sibling apps still have no import surface), **T-408** widens delete to every kind of created content (lyric versions, lyric documents, albums, projects -- the app has none of them today, and a lyric version a track points at is **refused** rather than deleted), and **T-409** carries a song title from Lyrics Studio through `GenerationSpec` to `Track.title` and the export filename (both title fields exist in the schema and neither is ever written). Order: T-404 -> T-405 -> T-408 -> T-409 -> T-406.
