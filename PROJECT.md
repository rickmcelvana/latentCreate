...
- **Next up (2026-09-05):** **T-512** -- strip the model catalog to the curated installable list. **T-507 is complete** (T-507a + T-507a-2 the frontend states sweep, both click-through-passed; T-507b the adopted-profile `comfy.models` enrichment, landed). Then the **2026-09-05 catalog pivot** (decisions log): the catalog cannot auto-install an arbitrary gallery model (comfy-mcp gives no URL; MCP-SURFACE §36), so it becomes a **curated installable list only**, and the app ships **five commercial-safe image profiles**. **T-511 is COMPLETE 2026-09-05** -- all five authored, every file's URL/size verified live on Hugging Face and every source ungated + commercial-safe, each installing one-click through the **existing Setup Models step**: **Flux.1 Schnell FP8** (Apache-2.0, 1 file), **Chroma1 HD** (Apache-2.0, 3 files), **SDXL 1.0** (Open RAIL++-M, 1 file), **SD 3.5 Large FP8** (Stability Community, 1 file), **Qwen-Image** (Apache-2.0, 4 files inc. an 8-step Lightning LoRA, `vram_gb_min` 24 -- the high-VRAM option, not run-verifiable on the 16 GB dev card). All pinned in `library`'s shipped-profiles guard test. **T-512** now removes the dead gallery browse (T-504 backend, T-505a/b UI, T-505d adopt) so the Models step is the whole catalog; import-your-own-workflow (T-313) stays. Then **T-508** installers, **T-509** THIRD-PARTY-LICENSES (carries all five image-model licences), **T-510** public-repo readiness. No feature work remains in the phase.
