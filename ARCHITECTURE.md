...
3. **Graph edits** happen on that working copy before the run, for the two cases slots cannot express: splicing `LoraLoaderModelOnly` nodes after the profile's `attach_after` node, and replacing a deprecated save node with `SaveAudioAdvanced` writing lossless — **conditionally**: ACE-Step's template ships `SaveAudioMP3`, MiniMax's already ships `SaveAudioAdvanced`, so the pipeline intervenes only when the template disagrees with the profile's `output` block (verified, MCP-SURFACE §6a). Shipping lossy MP3 into a mastering chain would undercut the whole suite. (Open: `SaveAudioAdvanced.format` is a V3 dynamic combo — mechanism TBD, MCP-SURFACE §5.)
