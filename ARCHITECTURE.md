...
Shipped audio profiles: **ace-step-1.5-turbo** (default; Apache-2.0, lyrics+vocals, LoRA ecosystem), **ace-step-1.5-xl-base** and **ace-step-1.5-xl-sft** (T-513; Apache-2.0, the non-distilled variants), **minimax-music-3**. **Stable Audio is backlog** (PROJECT.md decisions log 2026-09-05: SA 3.0's template is slot-unreliable, SA Open 1.0's `t5-base` encoder has no ungated ComfyUI source). Also backlog/excluded from the planned list: **musicgen** (CC-BY-NC, non-commercial), **yue** (24 GB+, not a gallery template — needs a hand-authored workflow), **diffrhythm**. Plus five commercial-safe image profiles for cover art (T-511): Flux.1 Schnell FP8, Chroma1 HD, SDXL 1.0, SD 3.5 Large FP8, Qwen-Image. The gallery also carries ACE-Step split variants and **v1 M2M editing + instrumentals** templates — the M2M one is the natural home for the backlog's audio-to-audio flows.
...
> **Extended, then narrowed (T-505d 2026-09-03, T-512 2026-09-05).** T-505d briefly gave this path a **second entry point** -- adopting a Model-catalog *gallery* row with one click -- but the **2026-09-05 catalog pivot removed the gallery** (comfy-mcp cannot install an arbitrary gallery model; PROJECT.md decisions log, MCP-SURFACE §36), so adoption is gone and **file import is the only entry point again**. What T-505d added *survives it*, because it is what lets this path carry a **non-audio** workflow and what the shipped **image** profiles rely on: emission **detects the output kind from the graph's save node** (`SaveImage` -> `kind: image`) instead of hardcoding `kind: music`, and the role suggester reads **conditioning polarity** rather than input names, because an image graph's positive and negative encoders are both `CLIPTextEncode` exposing an input named `text` -- a name-only match pre-ticks both and feeds the user's prompt into the negative side (unreachable by name matching, the same class of failure the pre-tick rule exists to prevent). Evidence: [docs/MCP-SURFACE.md](docs/MCP-SURFACE.md) §34.
>
> **Readiness of an imported profile (updated T-507b, 2026-09-05).** `emit_profile` now **populates `comfy.models`** from the graph's model-loader COMBO slots, resolving each filename's folder against a live inventory, so an imported profile whose files are installed reads **Ready** on the Models step rather than "Cannot check". `source_url`/`size_bytes` stay `None` -- the app knows where a file *is*, never claims it can fetch someone else's weights. Everything else -- picker entry, controls, generation, provenance -- is identical to a shipped profile.
...
1. **ComfyUI** — detect `comfy-mcp` on PATH (offer install instructions `pip install comfy-mcp` if missing), detect/launch ComfyUI (`launch_comfyui`), show server info. ⚠ **Local only, as built.** The card assumes the ComfyUI on this machine: `src-tauri/src/comfy.rs` builds a `LocalComfy` unconditionally and nothing reads `ComfyConfig.mode` or `ComfyConfig.url`, though both — and a whitelisted `comfy_cloud_api_key` keychain secret — exist in the schema. A mode control, an address field and the key entry are **T-515**, a v1 blocker scheduled as its own session (PROJECT.md decisions log, 2026-09-06): the live check comes first, because comfy-mcp's `comfy_target` (self-hosted remote) is a different product from Comfy Cloud, and `download_model` refuses with a remote target configured — so curated install is local-only and the Models step will have to say so.
2. **Models** (§10a) — the curated **installable list**: every shipped profile, **audio and image**, shows "Installed" ✅ / "Not installed" by matching its declared files against the local install (`search_models`), with one-click install (`download_model` + `download` progress streamed; refuses outright when a remote ComfyUI target is configured). The step is split **Audio | Image** by profile kind. **It is also where the default model for each studio is chosen** (T-514): every row is radio-selectable and writes `default_profile_id` / `default_image_profile_id`. This is where a user brings a model in without hand-importing a workflow; anything off the curated list is imported via §5b, whose importer lives on this step too.
...
4. **Cover art** — image models are installed on the Models step (2), not a separate step; the CoverArt view generates artwork over the chosen image profile (T-506).
...
### 10a. The model catalog — a curated installable list

**The reason the Setup page exists beyond LLM selection: a user brings new audio and image models into ComfyUI from the app.** Verified live (2026-09-02, re-confirmed 2026-09-05 — [docs/MCP-SURFACE.md §32–§33, §36](docs/MCP-SURFACE.md)): comfy-mcp has **no model-hub search**, and a gallery template hands out **no download URL** (`download_model` is URL-only; `local_check` names the missing file but not its source; ComfyUI-Manager's model DB is not exposed). So the app can only install models whose URLs it **curates**. The 2026-09-05 pivot (PROJECT.md decisions log) drew the consequence: **the catalog is the curated installable set, and nothing else** — there is no gallery browser, because loading readiness for ~160 rows nobody can install is slow and pointless.

- **The catalog *is* the Setup Models step.** Every shipped profile (`profiles/*.json`, audio and image) is rendered there with a readiness pill and one-click Install, split `Audio | Image` by `profile.kind`. Readiness is the **profile-file-list match** (`search_models(folder=)`), *not* `local_check.runnable` — a shipped profile's `slot_overrides` make a working install report `runnable:false` (the MiniMax lesson, MCP-SURFACE §6). Install is `download_model` on each file's hand-verified, **ungated** `source_url` through the `download` progress path (`src-tauri/src/install.rs`).
- **Curated set (T-511, T-513).** Four audio (ACE-Step 1.5 Turbo / XL Base / XL SFT, MiniMax Music 3) and five commercial-safe image models (Flux.1 Schnell FP8, Chroma1 HD, SDXL 1.0, SD 3.5 Large FP8, Qwen-Image). Each `ModelFileSpec` carries `file`/`folder`/`source_url`/`size_bytes`; the licence is shown per row (CONVENTIONS) and every curated model is commercial-use-safe.
- **Anything off the curated list** is brought in by **import-your-own-workflow** (§5b), the bring-your-own valve — a file picker and a role-mapping screen, not a gallery.

This introduces **no raw HTTP** (OQ-3 stays NO) and ships **no models** — only the user's own ComfyUI fetches, from URLs the app curated. The paid hosted/cloud tier (`list_partner_models` / `partner_generate`) remains a documented later addition (backlog, ROADMAP). **History:** T-504/T-505 built a gallery-browse-and-adopt catalog; **T-512 (2026-09-05) removed it** once the pivot established that an arbitrary gallery model cannot be installed.
