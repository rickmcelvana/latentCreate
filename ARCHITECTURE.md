...
- **Browse** the gallery (`search_templates` with `type` + `exclude_api:true` — image ~163 rows, audio ~19), split by `output_type` (`audio` for step 2, `image` for step 4). One shared component, two filters. The paid `api:true` hosted tier is out of v1 (Non-goals).
