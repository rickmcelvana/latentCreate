...
- **Curated set (T-511).** Two audio (ACE-Step, MiniMax) and five commercial-safe image models (Flux.1 Schnell FP8, Chroma1 HD, SDXL 1.0, SD 3.5 Large FP8, Qwen-Image). Each `ModelFileSpec` carries `file`/`folder`/`source_url`/`size_bytes`; the licence is shown per row (CONVENTIONS) and every curated model is commercial-use-safe.
