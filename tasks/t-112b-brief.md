# T-112b: the LLM step's Tauri commands
**Depends:** T-112a, T-108, T-109 | **Crate/dir:** `src-tauri`
**Files to create/modify:**
- `src-tauri/src/llm.rs` (create)
- `src-tauri/src/lib.rs` (modify: one `mod`, two commands, `DataDir` state, `shipped_dir` helper)
- `src-tauri/Cargo.toml` (modify: one dependency)
- `src-tauri/tauri.conf.json` (modify: one resources entry)

## Goal
Report what the endpoint offers, and prove it answers. Read
[LLM-SURFACE](../docs/LLM-SURFACE.md) **section 11** first -- every rule below is a captured
fact.

## Spec
Exactly the reference implementation below.

**This is where T-109's `ollama_native` work pays off.** The OpenAI-compatible list returns ids
and nothing else. Measured on the verification machine: 13 models, of which **2 cannot chat at
all** and **8 run on someone else's servers**, and `/v1/models` presents all 13 identically.

**Three rules that are correctness:**

- **Unknown is not false.** Against a non-Ollama endpoint no capability can be checked, so every
  field is `None`. Reporting `is_remote: false` would tell a user their lyrics stay on their
  machine when nobody verified it. That is the one mistake this step must not make.
- **Unknown is not unusable either.** A model whose capabilities could not be read stays
  selectable; hiding them strands a user on LM Studio with an empty picker.
- **The test call's success criterion is a well-formed response, not non-empty content.** A
  reasoning model spends the token budget on chain-of-thought first: 20 tokens produced empty
  content and `finish_reason: length` on a *healthy* endpoint (LLM-SURFACE 2, 11.4). Asserting
  non-empty content reports a broken setup to a user whose setup is fine.

**One keychain read for the whole step.** `has_key` rides on the status because answering it
means reading the secret, and on macOS a read can raise a prompt (T-004). The frontend must
never call `has_secret` itself. **The key value never crosses the boundary** -- not in the
payload, not in an error.

**A missing `/v1` gets a real next step.** It answers 404 with the plain-text body
`404 page not found`, and it is the likeliest thing a user gets wrong (LLM-SURFACE 11.3).

## The live tests
Two `#[ignore]` tests are included. They are **not** run by CI or the gate. Both have been run
and pass; leave them exactly as written -- `test_the_test_call_succeeds_on_a_thinking_model` is
the one that pins the reasoning-budget trap above.
...
### `src-tauri/src/lib.rs`, `Cargo.toml`, `tauri.conf.json` (modify)
Note `shipped_profiles_dir` becomes the generic `shipped_dir(app, name)`, used for both
`profiles/` and the new `data/`. `data/` is deliberately **not** inside `profiles/`, which is
scanned for model profiles and would report a non-profile file there as malformed.
```diff
diff --git a/Cargo.lock b/Cargo.lock
index 91ba4c2..febce64 100644
--- a/Cargo.lock
+++ b/Cargo.lock
@@ -80,6 +80,7 @@ name = "app"
 version = "0.1.0"
...
index 458572c..8a6233c 100644
--- a/src-tauri/Cargo.toml
+++ b/src-tauri/Cargo.toml
@@ -25,6 +25,7 @@ log = "0.4"
 tokio = { version = "1.53", features = ["time", "sync"] }
 
 create-core = { path = "../crates/create-core" }
+futures-util = { version = "0.3", default-features = false }
 library = { path = "../crates/library" }
 mcp-bridge = { path = "../crates/mcp-bridge" }
 llm-bridge = { path = "../crates/llm-bridge" }
diff --git a/src-tauri/src/lib.rs b/src-tauri/src/lib.rs
index b2001c8..f02f486 100644
...
+mod llm;
 ...
 use jobs::ComfyState;
@@ -17,6 +18,12 @@ use jobs::ComfyState;
...
## Acceptance criteria
- `npm run gate` green.
- `cargo test -p app` goes 20 -> **29** (26 passing, 3 ignored).
- `cargo test -p app -- --ignored` is **not** run by the gate.
- **No non-ASCII characters anywhere in the diff.**

## Out of scope
All frontend (T-112c/d). Do not modify `llm-bridge` -- `OllamaModel::can_chat` / `thinks` /
`is_remote` and `from_openai_base_url` already exist from T-109.

## If unclear
Follow the reference implementation exactly. In particular do not "simplify" the `Option<bool>`
capabilities to `bool`.

## Aider launch
```bash
aider --no-auto-commits --model ollama_chat/kimi-k2.7-code:cloud --read WORKFLOW.md --read CONVENTIONS.md --read docs/LLM-SURFACE.md --read crates/llm-bridge/src/ollama.rs --read crates/llm-bridge/src/openai.rs --read crates/create-core/src/suggestions.rs --file src-tauri/src/llm.rs --file src-tauri/src/lib.rs --file src-tauri/Cargo.toml --file src-tauri/tauri.conf.json
```
