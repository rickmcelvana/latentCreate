...
| `minimax-music-3` ✅**verified** | Flagship quality; full songs up to 5 min | **`caption`** + lyrics ⚠ | none found | int8 weights on a 16 GB card | open weights, **conditional** ⚠ | Template `audio_minimax_music_3` (2026-08-13, native/free). Installed 2026-08-23 as **int8**; template hardcodes the **fp16** DiT, so the profile must override `37/6.unet_name` — with that one change `validate_workflow` returns clean (MCP-SURFACE §6). Subgraph addressing (`37/...`), **three seeds**, **two duration fields that ship disagreeing** (60 vs 120), and it already uses `SaveAudioAdvanced`. License allows commercial use **with attribution**; separate agreement above ~$20M revenue — surface both in UI. ⚠ Not the same as MiniMax **H3** (video) |
