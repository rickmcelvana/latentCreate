...
- Styling: plain CSS in `theme.css`, dark professional theme. **Palette tracks `latentbeats.com`** (`../website/latentbeats.com/css/style.css`) — the umbrella brand, blue since Aug 2026: `--bg: #0a0e1a`, `--accent: #58a6ff`, `--radius: 12px`. The sibling *apps* still carry the older GitHub-dark ground; latentCreate deliberately follows the site instead. Do not fork token values — change the site, then follow it. No UI framework (no MUI/AntD/Tailwind). Every className used in TSX has a rule in `theme.css` — **enforced by `app/src/theme.test.ts`**, which fails the gate on a class with no rule behind it (including the ones that only appear inside a template literal's `${...}`). A class that is genuinely styled by its parent goes in that file's `UNSTYLED_BY_DESIGN` map **with its reason**; a class that looks unstyled on screen belongs in `theme.css` instead. Written after three spacing bugs of one shape reached a person in a single day (PROJECT.md, 2026-09-06).
