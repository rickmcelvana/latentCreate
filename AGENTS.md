...
5. `tasks/ROADMAP.md` → the current `tasks/phase-N.md` — pick up the first unfinished T-number. **Phases 0 and 1 are complete**, tagged `phase0-done` and `phase1-done` (2026-08-25); [tasks/phase-1.md](tasks/phase-1.md) is the record of the latter, not a to-do list. **Phase 2, Lyrics Studio (T-201 onwards), is next** and has **no briefs yet** — they are written at the start of a phase, not ahead of it, because each phase's interfaces harden in review. PROJECT.md's Snapshot carries the live state — trust it over this line if they ever disagree.
