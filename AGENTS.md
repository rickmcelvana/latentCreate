...
5. `tasks/ROADMAP.md` → the current `tasks/phase-N.md` — pick up the first unfinished T-number. **Phases 0 and 1 are complete**, tagged `phase0-done` and `phase1-done` (2026-08-25); [tasks/phase-1.md](tasks/phase-1.md) is the record of the latter, not a to-do list. **Phase 2, Lyrics Studio, is in progress** — T-201 … T-210 have landed (through the consent-gated prompt optimizer and the shared `<PromptDiff>`); **only T-211, the live milestone, remains**. Briefs are still written at the start of a phase, not ahead of it. PROJECT.md's Snapshot carries the live state — trust it over this line if they ever disagree.
